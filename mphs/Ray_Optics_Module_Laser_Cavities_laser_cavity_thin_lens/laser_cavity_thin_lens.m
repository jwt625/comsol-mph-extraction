...
model.param.set('L', '0.1[m]', 'Mirror-lens distance');
model.param.set('D', '25[mm]', 'Mirror diameter');
model.param.set('f', '0.5[m]', 'Lens focal length');
model.param.set('th', '1e-3[deg]', 'Initial ray angle');
model.param.set('n', '1.5', 'Lens index');
model.param.set('R', 'f*2*(n-1)', 'Radius of curvature (thin biconvex lens)');
model.param.set('T0', '1[us]', 'Total computation time');
model.param.set('dt', 'T0/100', 'Time increment');

model.func.create('rect1', 'Rectangle');
model.func('rect1').model('comp1');
model.func('rect1').set('lower', 0);
model.func('rect1').set('upper', '2*f');
model.func('rect1').set('smooth', 0.001);

model.geom.load({'part1'}, 'Ray_Optics_Module/3D/Spherical_Lenses/spherical_equi_convex_lens_3d.mph', {'part1'});
model.geom('geom1').create('pi1', 'PartInstance');
model.geom('geom1').feature('pi1').set('selkeepnoncontr', false);
model.geom('geom1').feature('pi1').set('part', 'part1');
model.geom('geom1').feature('pi1').setEntry('inputexpr', 'R', 'R');
model.geom('geom1').feature('pi1').setEntry('inputexpr', 'Tc', '2[mm]');
...
model.physics('gop').feature('mir1').selection.set([4 11]);
...
model.mesh('mesh1').feature('ftet1').feature('size1').selection.set([7 8]);
...
model.mesh('mesh1').feature('ftet1').feature('size1').set('hmax', 'D/10');
model.mesh('mesh1').feature('ftet1').feature('size1').set('hminactive', true);
model.mesh('mesh1').feature('ftet1').feature('size1').set('hmin', 'D/20');
...
model.batch('p1').set('plistarr', {'range(0.1,0.1,1.2)'});
...
model.result('pg1').set('title', 'Ray trajectory, L=0.1');
...
model.title('Laser Cavity with a Thin Lens');

model.description('This is an example of a simple laser cavity consisting of two flat mirrors with a thin biconvex lens between them. A ray, initially released at a small angle relative to the optical axis, is traced over a large number of reflections to determine whether it remains confined within the cavity. The study is repeated for several cavity sizes and the stability analysis is then compared to ABCD matrix theory.');
