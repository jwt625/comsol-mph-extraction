function out = model
%
% force_calculation_03_magnetic_torque_bem.m
%
...
model.geom('geom1').insertFile('force_calculation_01_introduction.mph', 'geom2');
...
model.param.set('cf', '2-1/sqrt(2)', 'Correction factor, as used in the force validation model');
model.param.set('qm', 'sqrt(4*pi*mu0_const*1[m/H]/cf)*1[Wb]', 'Amount of magnetic charge on the poles, as used in the force validation model');
model.param.set('Br', 'qm/Ap', 'Remanent flux density, as used in the force validation model');
model.param.set('Brx', 'Br*sin(Ra)', 'Remanent flux density (x component)');
model.param.set('Brz', 'Br*cos(Ra)', 'Remanent flux density (z component)');
model.param.set('mu', '(qm/mu0_const)*1[m]', 'Magnetic dipole moment of a magnetized rod of 1[m] length, having an amount of magnetic charge on its poles equal to ''qm''');
model.param.set('Bez', '1[N*m]/mu', 'External field needed to achieve 1[N*m] of torque, assuming a perpendicular magnetized rod having a magnetic dipole moment equal to ''mu''');
...
model.selection('sel3').set([17 19 22 24 42 43 44 45]);
...
model.selection('sel5').label('External Field In');
...
model.selection('sel6').label('External Field Out');
...
model.selection('sel7').label('Exterior Cube Surface');
model.selection('sel7').geom(2);
model.selection('sel7').set([1 2 3 4 5 54]);
...
model.physics('mfncbe').selection.set([-1 1 2 3 4]);
...
model.physics('mfncbe').feature('mfc3').set('normBr_crel_BH_RemanentFluxDensity', 'sqrt(Brx^2+Brz^2)');
model.physics('mfncbe').feature('mfc3').set('e_crel_BH_RemanentFluxDensity', {'Brx/sqrt(Brx^2+Brz^2)' '0' 'Brz/sqrt(Brx^2+Brz^2)'});
...
model.physics('mfncbe').feature('fcal1').set('TorqueAxis', [0 1 0]);
...
model.physics('mfncbe').feature('fcal2').set('TorqueAxis', [0 1 0]);
model.physics('mfncbe').create('mflx1', 'MagneticFluxDensity', 2);
model.physics('mfncbe').feature('mflx1').selection.named('sel5');
model.physics('mfncbe').feature('mflx1').set('nB', 'Bez');
model.physics('mfncbe').create('zsp1', 'ZeroMagneticScalarPotential', 2);
model.physics('mfncbe').feature('zsp1').selection.named('sel6');
...
model.physics('mfnc').feature('mfc2').set('normBr_crel_BH_RemanentFluxDensity', 'sqrt(Brx^2+Brz^2)');
model.physics('mfnc').feature('mfc2').set('e_crel_BH_RemanentFluxDensity', {'Brx/sqrt(Brx^2+Brz^2)' '0' 'Brz/sqrt(Brx^2+Brz^2)'});
...
model.physics('mfnc').feature('fcal1').set('TorqueAxis', [0 1 0]);
...
model.physics('mfnc').feature('fcal2').set('TorqueAxis', [0 1 0]);
model.physics('mfncbe2').selection.set([-1]);
model.physics('mfncbe2').create('mflx1', 'MagneticFluxDensity', 2);
model.physics('mfncbe2').feature('mflx1').selection.named('sel5');
model.physics('mfncbe2').feature('mflx1').set('nB', 'Bez');
model.physics('mfncbe2').create('zsp1', 'ZeroMagneticScalarPotential', 2);
model.physics('mfncbe2').feature('zsp1').selection.named('sel6');
...
model.multiphysics('msspc1').selection.all;
...
model.mesh('mesh1').feature('map1').selection.named('sel7');
...
model.result.numerical('gev1').setIndex('expr', 'sqrt(mfncbe.Tx_BEM_rod^2+(mfncbe.Ty_BEM_rod-1[N*m])^2+mfncbe.Tz_BEM_rod^2)', 0);
model.result.numerical('gev1').setIndex('descr', 'Magnetic torque error (BEM rod)', 0);
model.result.numerical('gev1').setIndex('expr', 'sqrt(mfncbe.Tx_BEM_probe^2+(mfncbe.Ty_BEM_probe-1[N*m])^2+mfncbe.Tz_BEM_probe^2)', 1);
model.result.numerical('gev1').setIndex('descr', 'Magnetic torque error (BEM probe)', 1);
...
model.result.numerical('gev2').setIndex('expr', 'sqrt(mfnc.Tx_FEM_rod^2+(mfnc.Ty_FEM_rod-1[N*m])^2+mfnc.Tz_FEM_rod^2)', 0);
model.result.numerical('gev2').setIndex('descr', 'Magnetic torque error (FEM rod)', 0);
model.result.numerical('gev2').setIndex('expr', 'sqrt(mfnc.Tx_FEM_probe^2+(mfnc.Ty_FEM_probe-1[N*m])^2+mfnc.Tz_FEM_probe^2)', 1);
model.result.numerical('gev2').setIndex('descr', 'Magnetic torque error (FEM probe)', 1);
...
model.result('pg5').label('Magnetic Torque Error');
model.result('pg5').set('titletype', 'manual');
model.result('pg5').set('title', 'Mesh convergence of magnetic torque');
...
model.title(['Force Calculation 3 ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' Magnetic Torque BEM FEM']);

model.description(['This model is a continuation of the Magnetic Force BEM FEM verification model. A single magnetized rod of one meter length, is placed in a perpendicular external field. The relative permeability is assumed to be one everywhere. The strength of the external field is chosen such that the analytical model predicts a torque on the rod, of exactly one Newton-meter.' newline  newline 'The boundary element method (BEM) and the finite element method (FEM) are compared to the analytical model for several mesh sizes. The quality of the Maxwell surface stress tensor on the poles is inspected. A parametric sweep is used to investigate mesh convergence for both BEM and FEM.']);
