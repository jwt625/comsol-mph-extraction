...
model.param.set('theta0', '0[deg]');
model.param.descr('theta0', 'Load direction angle');

model.coordSystem.create('sys2', 'geom1', 'Cylindrical');
model.coordSystem('sys2').setIndex('origin', 'PinHoleY', 1);
model.coordSystem('sys2').setIndex('axis', 1, 0);
model.coordSystem('sys2').setIndex('axis', 0, 2);
model.coordSystem('sys2').setIndex('radialbasevector', 0, 0);
model.coordSystem('sys2').setIndex('radialbasevector', 'sin(theta0)', 1);
model.coordSystem('sys2').setIndex('radialbasevector', 'cos(theta0)', 2);

model.func('an1').set('expr', 'F*cos(p)');
model.func('an1').set('args', 'F, p');
model.func('an1').setIndex('argunit', 'Pa', 0);
model.func('an1').setIndex('argunit', 'rad', 1);

model.physics('solid').feature('bndl1').set('coordinateSystem', 'sys2');
model.physics('solid').feature('bndl1').set('FperArea', {'load(P0*sign(X),sys2.phi)*(sign(X)*(abs(sys2.phi)-pi/2)<0)' '0' '0'});
...
model.study('std1').feature('stat').setIndex('plistarr', '', 0);
model.study('std1').feature('stat').setIndex('punit', 'Pa', 0);
model.study('std1').feature('stat').setIndex('pname', 'theta0', 0);
model.study('std1').feature('stat').setIndex('plistarr', 'range(0,10,160)', 0);
...
model.result('pg6').run;
model.result('pg6').label('Reaction Force, X-component');
model.result('pg6').set('ylabelactive', true);
model.result('pg6').set('ylabel', 'Reaction Force (N)');
model.result('pg6').run;
model.result('pg6').feature('tblp1').set('plotcolumninput', 'manual');
model.result('pg6').feature('tblp1').set('plotcolumns', [2 5 8 11]);
...
model.result('pg7').label('Reaction Force, Y-component');
model.result('pg7').run;
model.result('pg7').feature('tblp1').set('plotcolumns', [3 6 9 12]);
model.result('pg7').run;
model.result('pg7').run;
model.result.duplicate('pg8', 'pg7');
model.result('pg8').run;
model.result('pg8').label('Reaction Force, Z-component');
model.result('pg8').run;
model.result('pg8').feature('tblp1').set('plotcolumns', [4 7 10 13]);
model.result('pg8').run;
model.result.create('pg9', 'PlotGroup1D');
model.result('pg9').run;
model.result('pg9').label('Stress as Function of Load Angle');
...
model.result('pg9').feature('ptgr1').selection.set([86 88 103]);
model.result('pg9').feature('ptgr1').set('expr', 'solid.mises');
model.result('pg9').feature('ptgr1').set('unit', 'MPa');
model.result('pg9').feature('ptgr1').set('linemarker', 'cycle');
model.result('pg9').feature('ptgr1').set('legend', true);
model.result('pg9').feature('ptgr1').set('legendmethod', 'evaluated');
model.result('pg9').feature('ptgr1').set('legendpattern', '(eval(X,mm), eval(Y,mm), eval(Z,mm))');
model.result('pg9').run;
model.result('pg9').run;
...
model.title(['Bracket ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' Parametric Analysis']);

model.description('This example illustrates how to set up several structural load cases and compares the results. The model extends bracket_static.mph.');

model.mesh.clearMeshes;

model.sol('sol1').clearSolutionData;

model.label('bracket_parametric.mph');
