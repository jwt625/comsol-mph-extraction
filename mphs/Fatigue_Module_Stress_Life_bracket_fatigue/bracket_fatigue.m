...
model.param.descr('k', 'Stress factor');

model.func('an1').set('expr', 'para*F*cos(atan2(py,abs(px)))');
...
model.study('std1').feature('stat').setIndex('pname', 'elSize', 0);
...
model.func('int1').setIndex('funcs', 'wohler', 0, 0);
...
model.physics('ftg').feature('slif1').selection.all;
model.physics('ftg').feature('slif1').set('stress', 'stressMisesPrincipal');
model.physics('ftg').feature('slif1').set('ftgSNModification', 'stressFactor');
model.physics('ftg').feature('slif1').set('fatigueInputPhysics', 'solid');
model.physics('ftg').feature('slif1').set('lifeCurve', 'wohler');
model.physics('ftg').feature('slif1').set('k', 'k');
model.physics('ftg').feature('slif1').set('Ncut', '1e6');
...
model.study('std2').feature('param').setIndex('pname', 'elSize', 0);
...
model.sol('sol2').feature('s1').feature('p1').set('pname', {'k'});
model.sol('sol2').feature('s1').feature('p1').set('plistarr', {'0.7 0.28'});
...
model.sol('sol2').feature('s1').set('control', 'ftge');
...
model.title(['Bracket ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' Fatigue Evaluation']);

model.description(['The S' native2unicode(hex2dec({'20' '13'}), 'unicode') 'N curve, also called the W' native2unicode(hex2dec({'00' 'f6'}), 'unicode') 'hler curve, is one of the most popular methods for fatigue evaluation. When the operational conditions differ from the experimental, the S' native2unicode(hex2dec({'20' '13'}), 'unicode') 'N curve must be modified. This is demonstrated in the example where the effect of environmental conditions and surface finish is accounted for.']);
