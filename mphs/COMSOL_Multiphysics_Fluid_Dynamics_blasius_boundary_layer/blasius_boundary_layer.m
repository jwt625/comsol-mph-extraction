...
model.physics.create('c', 'CoefficientFormPDE', 'geom1', {'f' 'fprime'});
model.physics('c').prop('EquationForm').set('form', 'Automatic');
model.physics('c').field('dimensionless').field('f');
...
model.param.descr('xE', 'Evaluation location');
model.param.set('b0', 'nu/U0');
model.param.descr('b0', 'B-L scale');
model.param.set('N', '1');
model.param.descr('N', 'Mesh refinement factor');
...
model.physics('c').feature('cfeq1').setIndex('c', -2, 3);
model.physics('c').feature('cfeq1').setIndex('a', 1, 2);
model.physics('c').feature('cfeq1').setIndex('f', 0, 0);
model.physics('c').feature('cfeq1').setIndex('f', 0, 1);
model.physics('c').feature('cfeq1').setIndex('da', 0, 0);
model.physics('c').feature('cfeq1').setIndex('da', 0, 3);
model.physics('c').feature('cfeq1').setIndex('be', -1, 0);
model.physics('c').feature('cfeq1').setIndex('be', 'f', 3);
model.physics('c').create('dir1', 'DirichletBoundary', 0);
model.physics('c').feature('dir1').selection.set([1]);
model.physics('c').create('dir2', 'DirichletBoundary', 0);
model.physics('c').feature('dir2').selection.set([2]);
model.physics('c').feature('dir2').setIndex('useDirichletCondition', 0, 0);
model.physics('c').feature('dir2').setIndex('r', 1, 1);
...
model.mesh('mesh1').feature('edg1').feature('dis1').set('elemcount', 10000);
...
model.study('std1').label('Similarity Solution');
...
model.result('pg1').feature('lngr1').setIndex('legends', 'Similarity Solution', 0);
model.result('pg1').run;
...
model.cpl('genext1').set('dstmap', {'root.y/sqrt(b0*root.x)'});
...
model.geom('geom2').feature('r1').set('size', [3.1 0.5]);
model.geom('geom2').feature('r1').set('pos', [-1 0]);
model.geom('geom2').run('fin');
model.geom('geom2').run('r1');
model.geom('geom2').create('pt1', 'Point');
model.geom('geom2').run('fin');
model.geom('geom2').run('pt1');
model.geom('geom2').create('pt2', 'Point');
model.geom('geom2').feature('pt2').setIndex('p', 0.5, 1);
model.geom('geom2').run('fin');
...
model.physics('spf').feature('open1').selection.set([3 5]);
...
model.mesh('mesh2').feature('map1').feature('dis1').selection.set([4 5]);
model.mesh('mesh2').feature('map1').feature('dis1').set('numelem', '21*N');
model.mesh('mesh2').feature('map1').create('dis2', 'Distribution');
model.mesh('mesh2').feature('map1').feature('dis2').selection.set([2 3]);
model.mesh('mesh2').feature('map1').feature('dis2').set('numelem', '10*N');
model.mesh('mesh2').feature('map1').create('dis3', 'Distribution');
model.mesh('mesh2').feature('map1').feature('dis3').selection.set([1 6]);
model.mesh('mesh2').feature('map1').feature('dis3').set('type', 'predefined');
model.mesh('mesh2').feature('map1').feature('dis3').set('elemcount', '20*N');
model.mesh('mesh2').feature('map1').feature('dis3').set('elemratio', 15);
model.mesh('mesh2').feature('map1').feature('dis3').set('growthrate', 'exponential');
...
model.batch('p1').set('plistarr', {'1 2 4'});
...
model.sol('sol2').feature('s1').set('stol', '1e-5');
...
model.result.numerical('int1').set('expr', {});
...
model.title('The Blasius Boundary Layer');

model.description('This example simulates the Blasius boundary layer for different meshes and compares the results to the similarity solution.');
