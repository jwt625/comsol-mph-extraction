function out = model
%
% submarine_cable_08_inductive_effects_3d.m
%
...
model.param('par5').label('Thermal Parameters');

% To import content from file, use:
% model.param('par5').loadFile('FILENAME');
model.param('par5').set('Ntcon', '0.075281', 'Main conductor thermal packing density (phase)');
model.param('par5').set('Tmcon', '89.4[degC]', 'Average temperature of main conductors (phase)');
model.param('par5').set('Tmpbs', '82.7[degC]', 'Average screen temperature (lead sheath)');
model.param('par5').set('Tmarm', '70.1[degC]', 'Average temperature of armor ring');
model.param('par5').set('Tmext', '20[degC]', 'External temperature (sea bed)');
model.param('par5').set('Tmref', '20[degC]', 'Linearized resistivity reference temperature');
model.param('par5').set('R0cup', '1/Scup', ['Reference resistivity, copper, at 20' native2unicode(hex2dec({'00' 'b0'}), 'unicode') 'C']);
model.param('par5').set('R0pbs', '1/Spbs', ['Reference resistivity, lead sheath, at 20' native2unicode(hex2dec({'00' 'b0'}), 'unicode') 'C']);
model.param('par5').set('R0arm', '1/Sarm', ['Reference resistivity, armor wires, at 20' native2unicode(hex2dec({'00' 'b0'}), 'unicode') 'C']);
model.param('par5').set('ALcup', '3.9e-3[1/K]', 'Resistivity temperature coefficient, copper');
model.param('par5').set('ALpbs', '3.86e-3[1/K]', 'Resistivity temperature coefficient, lead sheath');
model.param('par5').set('ALarm', '5.3e-3[1/K]', 'Resistivity temperature coefficient, armor wires');

model.physics('mf').feature('coil1').set('ConstitutiveRelationJcE', 'LinearizedResistivity');
model.physics('mf').feature('coil1').set('minput_temperature_src', 'userdef');
model.physics('mf').feature('coil1').set('minput_temperature', 'Tmcon');
model.physics('mf').feature('coil2').set('ConstitutiveRelationJcE', 'LinearizedResistivity');
model.physics('mf').feature('coil2').set('minput_temperature_src', 'userdef');
model.physics('mf').feature('coil2').set('minput_temperature', 'Tmcon');
model.physics('mf').feature('coil3').set('ConstitutiveRelationJcE', 'LinearizedResistivity');
model.physics('mf').feature('coil3').set('minput_temperature_src', 'userdef');
model.physics('mf').feature('coil3').set('minput_temperature', 'Tmcon');
model.physics('mf').create('als2', 'AmperesLawSolid', 3);
model.physics('mf').feature('als2').label('Screens');
model.physics('mf').feature('als2').selection.named('dif1');
model.physics('mf').feature('als2').set('ConstitutiveRelationJcE', 'LinearizedResistivity');
model.physics('mf').feature('als2').set('minput_temperature_src', 'userdef');
model.physics('mf').feature('als2').set('minput_temperature', 'Tmpbs');
model.physics('mf').feature.duplicate('als3', 'als2');
model.physics('mf').feature('als3').label('Cable Armor');
model.physics('mf').feature('als3').selection.named('adj3');
model.physics('mf').feature('als3').set('minput_temperature', 'Tmarm');

model.material('mat1').propertyGroup.create('linzRes', 'Linearized_resistivity');
model.material('mat1').propertyGroup('linzRes').set('rho0', {'R0cup/Ncon'});
model.material('mat1').propertyGroup('linzRes').set('Tref', {'Tmref'});
model.material('mat1').propertyGroup('linzRes').set('alpha', {'ALcup'});
model.material('mat2').propertyGroup.create('linzRes', 'Linearized_resistivity');
model.material('mat2').propertyGroup('linzRes').set('rho0', {'R0pbs'});
model.material('mat2').propertyGroup('linzRes').set('Tref', {'Tmref'});
model.material('mat2').propertyGroup('linzRes').set('alpha', {'ALpbs'});
model.material('mat3').propertyGroup.create('linzRes', 'Linearized_resistivity');
model.material('mat3').propertyGroup('linzRes').set('rho0', {'R0arm'});
model.material('mat3').propertyGroup('linzRes').set('Tref', {'Tmref'});
model.material('mat3').propertyGroup('linzRes').set('alpha', {'ALarm'});
...
model.result.numerical('int1').setIndex('expr', 'mf.Qh/Lsec', 0);
model.result.numerical('int1').setIndex('unit', 'W/km', 0);
model.result.numerical('int1').setIndex('descr', 'Phase losses (linres 3D)', 0);
model.result.numerical('int1').set('table', 'tbl1');
model.result.numerical('int1').appendResult;
model.result.numerical('int2').setIndex('expr', 'mf.Qh/Lsec', 0);
model.result.numerical('int2').setIndex('unit', 'W/km', 0);
model.result.numerical('int2').setIndex('descr', 'Screen losses (linres 3D)', 0);
model.result.numerical('int2').set('table', 'tbl2');
model.result.numerical('int2').appendResult;
model.result.numerical('int3').setIndex('expr', 'mf.Qh/Lsec', 0);
model.result.numerical('int3').setIndex('unit', 'W/km', 0);
model.result.numerical('int3').setIndex('descr', 'Armor losses (linres 3D)', 0);
model.result.numerical('int3').set('table', 'tbl3');
model.result.numerical('int3').appendResult;
model.result.numerical('gev1').setIndex('expr', '(mf.RCoil_1/Lsec+mf.RCoil_2/Lsec+mf.RCoil_3/Lsec)/3', 0);
model.result.numerical('gev1').setIndex('unit', 'mohm/km', 0);
model.result.numerical('gev1').setIndex('descr', 'Phase AC resistance (linres 3D)', 0);
model.result.numerical('gev1').set('table', 'tbl4');
model.result.numerical('gev1').appendResult;
model.result.numerical('gev2').setIndex('expr', '(mf.LCoil_1/Lsec+mf.LCoil_2/Lsec+mf.LCoil_3/Lsec)/3', 0);
model.result.numerical('gev2').setIndex('unit', 'mH/km', 0);
model.result.numerical('gev2').setIndex('descr', 'Phase inductance (linres 3D)', 0);
model.result.numerical('gev2').set('table', 'tbl5');
model.result.numerical('gev2').appendResult;
model.result('pg8').run;
model.result('pg8').run;
...
model.title(['Submarine Cable 8c ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' Inductive Effects 3D']);
...
model.view('view4').set('locked', true);
model.view('view5').set('locked', true);
model.view('view1').camera.set('orthoscale', 0.3132075071334839);
model.view('view2').camera.set('orthoscale', 0.2982928454875946);
model.view('view3').camera.set('orthoscale', 0.2982928454875946);
model.view('view4').camera.set('orthoscale', 0.26664286851882935);

model.param('par3').set('Lsec', 'Nper*CPcab/Narm');

model.geom('geom1').run('fin');

model.view('view1').camera.set('zscale', 1);
model.view('view2').camera.set('zscale', 1);
model.view('view3').camera.set('zscale', 1);
model.view('view4').camera.set('zscale', 1);
model.view('view5').camera.set('zscale', 1);
model.view('view5').camera.setIndex('position', '1.8*Dcab', 0);
model.view('view5').camera.setIndex('position', '0.8*Dcab', 1);
model.view('view5').camera.setIndex('position', 'Lsec/(2*Nper)+0.5*Dcab', 2);
model.view('view5').camera.setIndex('up', 0, 0);
model.view('view5').camera.setIndex('up', 0, 1);
model.view('view5').camera.set('up', [0 0 1]);
model.view('view5').camera.setIndex('viewoffset', 0, 0);
model.view('view5').camera.set('viewoffset', [0 0]);

model.geom('geom1').feature('wp1').label('Phases, Screens, and Sea Bed');
model.geom('geom1').feature('wp1').set('quickz', '-Lsec/2+1e-6');
model.geom('geom1').run('wp1');
model.geom('geom1').feature('wp1').set('quickz', '-Lsec/2');
model.geom('geom1').feature('wp1').geom.feature.remove('csol1');
model.geom('geom1').feature('wp1').geom.run('rot2');
model.geom('geom1').feature('wp1').geom.create('c2', 'Circle');
model.geom('geom1').feature('wp1').geom.feature('c2').set('r', '5*Dcab/2');
model.geom('geom1').feature('wp1').geom.run('c2');
model.geom('geom1').feature('wp2').label('Cable Armor');
model.geom('geom1').run('wp2');
model.geom('geom1').feature('wp2').geom.feature.remove('c1');
model.geom('geom1').feature('wp2').geom.run('rot2');
model.geom('geom1').feature('wp2').geom.create('csol1', 'ConvertToSolid');
model.geom('geom1').feature('wp2').geom.feature('csol1').selection('input').set({'rot2'});
...
model.geom('geom1').feature('wp3').geom.create('ls6', 'LineSegment');
model.geom('geom1').feature('wp3').geom.feature('ls6').set('specify1', 'coord');
model.geom('geom1').feature('wp3').geom.feature('ls6').set('coord1', {'Darm/2-2*Tarm/3' '0'});
model.geom('geom1').feature('wp3').geom.feature('ls6').selection('vertex2').set('sca3(35)', 5);
model.geom('geom1').feature('wp3').geom.run('ls6');
model.geom('geom1').feature('wp3').geom.create('ls7', 'LineSegment');
model.geom('geom1').feature('wp3').geom.feature('ls7').set('specify1', 'coord');
model.geom('geom1').feature('wp3').geom.feature('ls7').set('coord1', {'Darm/2+2*Tarm/3' '0'});
model.geom('geom1').feature('wp3').geom.feature('ls7').selection('vertex2').set('sca3(39)', 5);
model.geom('geom1').feature('wp3').geom.run('ls7');
model.geom('geom1').feature('wp3').geom.create('rot8', 'Rotate');
model.geom('geom1').feature('wp3').geom.feature('rot8').selection('input').set({'ls6' 'ls7'});
model.geom('geom1').feature('wp3').geom.feature('rot8').set('rot', '360[deg]*range(1/Narm,1/Narm,1)');
model.geom('geom1').feature('wp3').geom.run('rot8');

model.view('view8').axis.set('xmin', 0.0868808776140213);
model.view('view8').axis.set('xmax', 0.11871876567602158);
model.view('view8').axis.set('ymin', -0.012240377254784107);
model.view('view8').axis.set('ymax', 0.012240377254784107);

model.geom('geom1').run('fin');

model.selection.remove('dif5');
model.selection.remove('cyl11');
model.selection.remove('dif6');
model.selection('cyl13').set('r', 'Darm/2+Tarm');
model.selection.create('cyl17', 'Cylinder');
model.selection('cyl17').model('comp1');

model.nodeGroup('grp1').add('selection', 'cyl17');

model.selection('cyl17').label('Copy Face 4');
model.selection('cyl17').set('entitydim', 2);
model.selection('cyl17').set('inputent', 'selections');
model.selection('cyl17').set('input', {'dif4'});
model.selection('cyl17').set('r', 'Darm/2');
model.selection('cyl17').set('rin', 'Darm/2-Tarm');
model.selection('cyl17').set('condition', 'allvertices');
model.selection.duplicate('cyl18', 'cyl17');

model.nodeGroup('grp1').add('selection', 'cyl18');

model.selection('cyl18').label('Copy Face 5');
model.selection('cyl18').set('r', 'Darm/2+Tarm');
model.selection('cyl18').set('rin', 'Darm/2');
model.selection.create('cyl19', 'Cylinder');
model.selection('cyl19').model('comp1');

model.nodeGroup('grp1').add('selection', 'cyl19');

model.selection('cyl19').label('Free Triangular 3');
model.selection('cyl19').set('entitydim', 2);
model.selection('cyl19').set('inputent', 'selections');
model.selection('cyl19').set('input', {'cyl17' 'cyl18'});
model.selection('cyl19').set('r', 'Tarm');
model.selection('cyl19').set('pos', {'Darm/2' 'Tarm/2' '0'});
model.selection('cyl19').set('condition', 'allvertices');
model.selection.create('int6', 'Intersection');
model.selection('int6').model('comp1');

model.nodeGroup('grp1').add('selection', 'int6');

model.selection('int6').label('Copy Face 4, Source Boundaries');
model.selection('int6').set('entitydim', 2);
model.selection('int6').set('input', {'cyl17' 'cyl19'});
model.selection.create('dif5', 'Difference');
model.selection('dif5').model('comp1');

model.nodeGroup('grp1').add('selection', 'dif5');

model.selection('dif5').label('Copy Face 4, Destination Boundaries');
model.selection('dif5').set('entitydim', 2);
model.selection('dif5').set('add', {'cyl17'});
model.selection('dif5').set('subtract', {'int6'});
model.selection.create('int7', 'Intersection');
model.selection('int7').model('comp1');

model.nodeGroup('grp1').add('selection', 'int7');

model.selection('int7').label('Copy Face 5, Source Boundaries');
model.selection('int7').set('entitydim', 2);
model.selection('int7').set('input', {'cyl18' 'cyl19'});
model.selection.create('dif6', 'Difference');
model.selection('dif6').model('comp1');

model.nodeGroup('grp1').add('selection', 'dif6');

model.selection('dif6').label('Copy Face 5, Destination Boundaries');
model.selection('dif6').set('entitydim', 2);
model.selection('dif6').set('add', {'cyl18'});
model.selection('dif6').set('subtract', {'int7'});
model.selection.create('dif7', 'Difference');
model.selection('dif7').model('comp1');

model.nodeGroup('grp1').add('selection', 'dif7');

model.selection('dif7').label('Free Triangular 4');
model.selection('dif7').set('entitydim', 2);
model.selection('dif7').set('add', {'cyl2'});
model.selection('dif7').set('subtract', {'int1' 'int2' 'int3' 'cyl10' 'cyl17' 'cyl18'});

model.mesh('mesh1').feature('map3').feature('size1').set('hmax', '2*Tpbs');
model.mesh('mesh1').feature('map3').feature('size1').set('hmin', '2*Tpbs');
model.mesh('mesh1').run('map3');
model.mesh('mesh1').feature('ftri3').selection.named('cyl19');
model.mesh('mesh1').feature('ftri3').feature('size1').selection.named('cyl19');
model.mesh('mesh1').feature('ftri3').feature('size1').set('hgradactive', false);
model.mesh('mesh1').run('ftri3');
...
model.mesh('mesh1').feature('cpf4').selection('source').named('int6');
model.mesh('mesh1').feature('cpf4').selection('destination').named('dif5');
model.mesh('mesh1').run('cpf4');
model.mesh('mesh1').run('ftri3');
model.mesh('mesh1').run('cpf4');
...
model.mesh('mesh1').feature('cpf5').selection('source').named('int7');
model.mesh('mesh1').feature('cpf5').selection('destination').named('dif6');
model.mesh('mesh1').run('cpf5');
model.mesh('mesh1').run('cpf4');
model.mesh('mesh1').run('cpf5');
model.mesh('mesh1').run('map4');
model.mesh('mesh1').feature('map4').selection.set([3232 3233 3243 3244]);
model.mesh('mesh1').feature('map4').selection.named('cyl10');
model.mesh('mesh1').create('ftri4', 'FreeTri');
model.mesh('mesh1').feature('ftri4').selection.named('dif7');
model.mesh('mesh1').feature('ftri4').create('size1', 'Size');
model.mesh('mesh1').feature('ftri4').feature('size1').set('custom', true);
model.mesh('mesh1').feature('ftri4').feature('size1').set('hminactive', true);
model.mesh('mesh1').feature('ftri4').feature('size1').set('hmin', 'Tarm');
model.mesh('mesh1').feature('ftri4').feature('size1').set('hgradactive', true);
model.mesh('mesh1').feature('ftri4').feature('size1').set('hgrad', 1.3);
model.mesh('mesh1').run('ftri4');
...
model.view('view5').set('transparency', true);
model.view('view5').set('renderwireframe', false);
model.view('view5').set('showaxisorientation', true);

model.mesh('mesh1').feature.remove('conv1');
...
model.result('pg4').run;
model.result.numerical('int1').setIndex('descr', 'Phase losses (linres 3D, short-periodic)', 0);
model.result.numerical('int1').set('table', 'tbl1');
model.result.numerical('int1').appendResult;
model.result.numerical('int2').setIndex('descr', 'Screen losses (linres 3D, short-periodic)', 0);
model.result.numerical('int2').set('table', 'tbl2');
model.result.numerical('int2').appendResult;
model.result.numerical('int3').setIndex('descr', 'Armor losses (linres 3D, short-periodic)', 0);
model.result.numerical('int3').set('table', 'tbl3');
model.result.numerical('int3').appendResult;
model.result.numerical('gev1').setIndex('descr', 'Phase AC resistance (linres 3D, short-periodic)', 0);
model.result.numerical('gev1').set('table', 'tbl4');
model.result.numerical('gev1').appendResult;
model.result.numerical('gev2').setIndex('descr', 'Phase inductance (linres 3D, short-periodic)', 0);
model.result.numerical('gev2').set('table', 'tbl5');
model.result.numerical('gev2').appendResult;
model.result('pg1').run;
...
model.result('pg2').run;
model.result('pg2').feature('mesh1').set('wireframecolor', 'black');
model.result('pg2').feature('mesh1').set('filteractive', false);
model.result('pg2').run;
model.result('pg3').run;
model.result('pg3').run;
model.result.dataset('cpl3').set('quickz', '-Lsec/2');
model.result.dataset('cpl3').set('genparadist', 'Lsec');
model.result('pg4').run;
model.result('pg4').run;
model.result('pg5').run;
model.result('pg5').run;
...
model.result('pg6').set('solutionintitle', false);
model.result('pg6').run;
model.result('pg6').set('solutionintitle', true);
model.result('pg6').set('titletype', 'auto');
model.result('pg7').run;
model.result('pg7').set('titletype', 'custom');
model.result('pg7').set('solutionintitle', false);
model.result('pg7').run;
model.result('pg7').set('solutionintitle', true);
model.result('pg7').set('titletype', 'auto');
model.result('pg8').run;
model.result('pg8').run;
model.result('pg4').run;
model.result('pg4').run;
...
model.view('view5').set('transparency', true);
model.view('view1').camera.set('orthoscale', 0.3294784724712372);
model.view('view2').camera.set('orthoscale', 0.3977237939834595);
model.view('view3').camera.set('orthoscale', 0.3977237939834595);
model.view('view4').camera.set('orthoscale', 0.3555237948894501);
model.view('view1').set('locked', false);
...
model.title(['Submarine Cable 8 ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' Inductive Effects 3D']);

model.description(['This last tutorial of the series intends to give a ' native2unicode(hex2dec({'20' '1c'}), 'unicode') 'final answer' native2unicode(hex2dec({'20' '1d'}), 'unicode') ' to 3D cable modeling. It has been developed with feedback from several experts from within the industry, and is on a par with the latest research (2020) when considering both performance and level of detail. Validation is included. The behavior of the models is analyzed within the context of several academic papers and the cable' native2unicode(hex2dec({'20' '19'}), 'unicode') 's official specifications.' newline  newline 'The tutorial is organized in five sections: First, the cable is modeled without twist and compared to the much more detailed 2D models. Then, the twist is added and the model is solved and verified a second time. A temperature correction is added in a third step and an experimental numerical stabilization method is demonstrated in a fourth step. The tutorial finishes by switching to a different kind of periodicity, reducing the size of the model a hundredfold while still providing the same accuracy.']);
...
model.label('submarine_cable_08_inductive_effects_3d.mph');
