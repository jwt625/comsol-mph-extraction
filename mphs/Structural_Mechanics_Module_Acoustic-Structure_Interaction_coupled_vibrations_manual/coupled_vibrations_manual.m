...
model.modelPath('/Applications/COMSOL62/Multiphysics/applications/Structural_Mechanics_Module/Acoustic-Structure_Interaction');
...
model.material('mat2').propertyGroup('Enu').set('E', {'2.1e11'});
...
model.physics('shell').feature('fl1').selection.set([3]);
model.physics('shell').feature('fl1').set('Ff_src', 'root.comp1.acpr.FAcoPerAreax');
model.physics('acpr').create('nacc1', 'NormalAcceleration', 2);
model.physics('acpr').feature('nacc1').selection.set([3]);
model.physics('acpr').feature('nacc1').set('Type', 'acc');
model.physics('acpr').feature('nacc1').set('acc_src', 'root.comp1.shell.us_ttx');
...
model.sol('sol3').feature('e1').feature('d1').label('Suggested Direct Solver (acpr) (Merged)');
model.sol('sol3').feature('e1').create('i1', 'Iterative');
model.sol('sol3').feature('e1').feature('i1').set('linsolver', 'gmres');
model.sol('sol3').feature('e1').feature('i1').label('Suggested Iterative Solver (GMRES with GMG) (acpr)');
model.sol('sol3').feature('e1').feature('i1').create('mg1', 'Multigrid');
model.sol('sol3').feature('e1').feature('i1').feature('mg1').set('prefun', 'gmg');
model.sol('sol3').feature('e1').create('i2', 'Iterative');
model.sol('sol3').feature('e1').feature('i2').set('linsolver', 'fgmres');
model.sol('sol3').feature('e1').feature('i2').label('Suggested Iterative Solver (FGMRES with GMG) (acpr)');
model.sol('sol3').feature('e1').feature('i2').create('mg1', 'Multigrid');
model.sol('sol3').feature('e1').feature('i2').feature('mg1').set('prefun', 'gmg');
...
model.result.evaluationGroup('std3EvgFrq').set('defaultPlotID', 'eigenfrequenciesTable_shell');
...
model.result('pg6').feature('con1').set('expr', {'acpr.p_t'});
...
model.result('pg8').feature('iso1').set('expr', {'acpr.p_t'});
...
model.result('pg8').label('Acoustic Pressure, Isosurfaces (acpr)');
model.result('pg5').run;
model.result('pg5').label('Mode Shape, Coupled Analysis');
...
model.result('pg6').label('Acoustic Pressure, Coupled Analysis');
model.result('pg7').run;
model.result('pg7').label('Sound Pressure Level, Coupled Analysis');
model.result('pg8').run;
model.result('pg8').label('Acoustic Pressure, Coupled Analysis, Isosurfaces');
model.result('pg5').run;

model.title('Vibrations of a Disk Backed by an Air-Filled Cylinder');

model.description('An eigenfrequency analysis of the vibration modes of a circular disk backed by a cylindrical cavity shows that you cannot neglect the coupling between the bending modes of the disk and the acoustic modes in the cylinder. The results correlate well with semi-analytical and experimental data.');
