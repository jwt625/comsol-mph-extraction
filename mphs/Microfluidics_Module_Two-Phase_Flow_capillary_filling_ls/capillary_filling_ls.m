...
model.multiphysics('ww1').selection.set([6 7]);
model.multiphysics('ww1').set('thetaw', '(3*pi/8)[rad]');

model.physics('ls').feature('lsm1').set('epsilon_ls', '5e-6');
...
model.physics('ls').feature('inl1').selection.set([8]);
model.physics('ls').feature('inl1').set('lscond', 'Fluid2ls');
model.physics('ls').create('out1', 'Outlet', 1);
model.physics('ls').feature('out1').selection.set([5]);
...
model.variable('var1').set('theta', '(acos(tpf1.intnormr))[1/deg]');
model.variable('var1').descr('theta', 'Contact angle expression');
...
model.probe('bnd1').set('expr', 'ls.Vf2');
...
model.title(['Capillary Filling ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' Level Set Method']);
