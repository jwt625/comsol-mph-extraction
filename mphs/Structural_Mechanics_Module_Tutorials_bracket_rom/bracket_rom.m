...
model.result.export('anim1').set('fontsize', '24');
...
model.result('pg2').set('window', 'window1');
model.result('pg2').run;
model.result('pg2').feature('tblp2').label('Unreduced Model');
...
model.study('std2').label('Model Reduction');
...
model.study('std2').feature('mr').set('qoiexpr', {'comp1.var1'});
model.study('std2').feature('mr').set('qoidescr', {'Pin displacement, x-component'});
model.study('std2').feature('mr').set('qoiexpr', {'comp1.var1' 'comp1.var2'});
model.study('std2').feature('mr').set('qoidescr', {'Pin displacement, x-component' 'Pin displacement, y-component'});
model.study('std2').feature('mr').set('qoiexpr', {'comp1.var1' 'comp1.var2' 'comp1.var3'});
model.study('std2').feature('mr').set('qoidescr', {'Pin displacement, x-component' 'Pin displacement, y-component' 'Pin displacement, z-component'});
...
model.sol('sol2').feature('v2').feature('comp1_solid_rig_disp').set('scalemethod', 'manual');
model.sol('sol2').feature('v2').feature('comp1_solid_rig_rot').set('scalemethod', 'manual');
model.sol('sol2').feature('v2').feature('comp1_u').set('scalemethod', 'manual');
model.sol('sol2').feature('v2').feature('comp1_solid_rig_disp').set('resscalemethod', 'parent');
model.sol('sol2').feature('v2').feature('comp1_solid_rig_rot').set('resscalemethod', 'parent');
model.sol('sol2').feature('v2').feature('comp1_solid_rig_disp').set('scaleval', '0.0042275879647855966');
model.sol('sol2').feature('v2').feature('comp1_solid_rig_rot').set('scaleval', '0.01');
model.sol('sol2').feature('v2').feature('comp1_u').set('scaleval', '1e-2*0.42275879647855963');
model.sol('sol2').feature('v2').set('notsolmethod', 'sol');
model.sol('sol2').feature('v2').set('notsol', 'sol2');
model.sol('sol2').feature('v2').set('control', 'mr');
model.sol('sol2').create('mor1', 'ModalReduction');
model.sol('sol2').feature('mor1').set('analysistype', 'transient');
model.sol('sol2').feature('mor1').set('control', 'mr');
model.sol('sol2').feature('v2').set('resscalemethod', 'manual');
model.sol('sol2').create('st3', 'StudyStep');
model.sol('sol2').feature('st3').set('study', 'std2');
model.sol('sol2').feature('st3').set('studystep', 'mr');
model.sol('sol2').feature('st3').set('useForModelReduction', false);
model.sol('sol2').feature('mor1').set('control', 'mr');
model.sol('sol2').feature('mor1').feature('aDef').set('cachepattern', true);
model.sol('sol2').feature('mor1').set('linearity', 'off');
model.sol('sol2').feature('mor1').set('analysistype', 'transient');
...
model.study('std3').feature('time').setEntry('activate', 'solid', false);
model.study('std3').feature('time').setEntry('activate', 'rom:rom1', true);
model.study('std3').feature('time').setEntry('reconstructors', 'solid', 'rom1');
model.study('std3').label('Reduced Model');
...
model.sol('sol4').feature('t1').set('tlist', 'range(0,0.2,10)');
...
model.sol('sol4').feature('t1').set('probes', {'var1' 'var2' 'var3'});
...
model.result('pg4').label('Stress (Reduced Model)');
model.result('pg4').run;
...
model.study('std1').label('Unreduced Model');

model.result('pg2').set('window', 'window1');
model.result('pg2').run;
model.result('pg2').label('Stress (Unreduced Model)');
model.result('pg4').run;
...
model.func('an1').set('periodicupper', '10[ms]');
model.func('an1').setIndex('argunit', 's', 0);

model.param.set('F_x', '400[N]*an1(t)');
model.param.descr('F_x', 'Applied force, x-component');
...
model.sol('sol4').feature('v1').set('control', 'time');
model.sol('sol4').create('t1', 'Time');
model.sol('sol4').feature('t1').set('tlist', '0 300');
model.sol('sol4').feature('t1').set('plot', 'off');
model.sol('sol4').feature('t1').set('plotgroup', 'pg1');
model.sol('sol4').feature('t1').set('plotfreq', 'tout');
model.sol('sol4').feature('t1').set('probesel', 'all');
...
model.title(['Bracket ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' Reduced-Order Modeling']);

model.description('This tutorial illustrates how to use a reduced-order model to compute the response of a structure subjected to three simultaneous loads having independent time histories. Reduced-order models provide an efficient way to analyze problems within linear structural dynamics.');
...
model.sol.remove('savepoint4_romd1');
model.sol.remove('savepoint5_romd1');
