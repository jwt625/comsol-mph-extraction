...
model.param.set('V0', '0[V]', 'Applied DC voltage');
model.param.set('Von', '0.896[V]', 'Applied DC voltage for the on state');
model.param.set('Voff', '-300[V]', 'Applied reverse voltage');
model.param.set('area', '1[mm^2]', 'Cross-section area of the PIN diode');
model.param.set('dJdt', '2e9[A/cm^2/s]', 'Desired current density ramp rate');
model.param.set('L0', 'abs(Voff)/(dJdt*area)', 'Inductance value');
model.param.set('dIdt', 'abs(Voff)/L0', 'Expected current ramp rate');
...
model.physics('semi').feature('smm1').create('mmfl1', 'FletcherMobilityModel', 1);
model.physics('semi').feature('smm1').set('mun_mat', 'root.comp1.semi.mun_fl');
model.physics('semi').feature('smm1').set('mup_mat', 'root.comp1.semi.mup_fl');
...
model.physics('semi').feature('mc2').set('V0', 'V_circ');
...
model.physics('cir').feature('I1').set('value', 'semi.I0_2');
model.physics('cir').create('ge1', 'GlobalEquations', -1);
model.physics('cir').feature('ge1').setIndex('name', 'V_circ', 0, 0);
model.physics('cir').feature('ge1').setIndex('equation', 'cir.I1_v-V_circ', 0, 0);
model.physics('cir').feature('ge1').set('DependentVariableQuantity', 'electricpotential');
model.physics('cir').create('L1', 'Inductor', -1);
model.physics('cir').feature('L1').setIndex('Connections', 1, 1, 0);
model.physics('cir').feature('L1').set('L', 'L0');
model.physics('cir').create('D1', 'Diode', -1);
model.physics('cir').feature('D1').setIndex('Connections', 2, 0, 0);
model.physics('cir').feature('D1').setIndex('Connections', 1, 1, 0);
...
model.physics('cir').feature('V2').set('value', 'Von+(Voff-Von)*rm1((t+10[ns])/10[ns])');

model.study('std1').feature('semie').set('useadvanceddisable', true);
model.study('std1').feature('semie').set('disabledphysics', {'cir/V2'});
model.study('std1').create('stat', 'Stationary');
model.study('std1').feature('stat').set('useadvanceddisable', true);
model.study('std1').feature('stat').set('disabledphysics', {'cir/V2'});
...
model.study('std1').feature('time').set('disabledphysics', {'cir/V1'});
...
model.sol('sol1').feature('s2').feature('fc1').set('minsteprecovery', 0.75);
...
model.sol('sol1').feature('t1').set('tlist', 'range(-20,1,0) range(10,10,500)');
...
model.result('pg5').label('Hole Concentration');
...
model.result('pg6').feature('glob1').setIndex('descr', 'Voltage at node 2 (applied)', 0);
model.result('pg6').feature('glob1').setIndex('expr', 'cir.v_1', 1);
model.result('pg6').feature('glob1').setIndex('unit', 'V', 1);
model.result('pg6').feature('glob1').setIndex('descr', 'Voltage at node 1 (PIN diode)', 1);
model.result('pg6').run;
...
model.result('pg8').set('ylabel', 'Applied and device voltage (V)');
