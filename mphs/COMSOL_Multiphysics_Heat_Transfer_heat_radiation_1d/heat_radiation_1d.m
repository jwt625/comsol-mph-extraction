...
model.geom('geom1').feature('i1').setIndex('coord', 0.1, 1);
model.geom('geom1').runPre('fin');
model.geom('geom1').run;

model.physics('ht').create('temp1', 'TemperatureBoundary', 0);
model.physics('ht').feature('temp1').selection.set([1]);
model.physics('ht').feature('temp1').set('T0', 1000);
model.physics('ht').create('sar1', 'SurfaceToAmbientRadiation', 0);
model.physics('ht').feature('sar1').selection.set([2]);
model.physics('ht').feature('sar1').set('epsilon_rad_mat', 'userdef');
model.physics('ht').feature('sar1').set('epsilon_rad', 0.98);
model.physics('ht').feature('sar1').set('Tamb', 300);
model.physics('ht').feature('solid1').set('k_mat', 'userdef');
model.physics('ht').feature('solid1').set('k', [55.563 0 0 0 55.563 0 0 0 55.563]);
model.physics('ht').feature('init1').set('Tinit', 1000);
...
model.title('Steady-State 1D Heat Transfer with Radiation');

model.description(['This is a benchmark problem for a 1D steady-state thermal analysis with a fixed temperature of 1000' native2unicode(hex2dec({'00' 'a0'}), 'unicode') 'K at the left end and radiation to 300' native2unicode(hex2dec({'00' 'a0'}), 'unicode') 'K at the right end. The temperature field from the analysis is compared with a NAFEMS benchmark solution.']);
