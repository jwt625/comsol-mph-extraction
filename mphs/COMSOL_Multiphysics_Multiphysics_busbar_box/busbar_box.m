...
model.result('pg3').set('showsolutionparams', 'on');
...
model.title('Electrical Heating in a Busbar');

model.description('This example analyzes the resistive heating of a busbar designed to conduct direct current.');
...
model.geom('geom1').feature('blk1').set('size', {'L+5*tbb' '3*L' 'L+6*tbb'});
model.geom('geom1').feature('blk1').set('pos', {'-2*tbb' '-L' '-tbb'});
model.geom('geom1').run('blk1');
model.geom('geom1').run('fin');

model.view('view1').set('transparency', true);

model.title('Convective Cooling of a Busbar');

model.description('This is a template MPH-file that you can use as the starting point for extending the example Electrical Heating in a Busbar with a study of the convective cooling of the busbar by an air stream.');

model.label('busbar_box.mph');
