...
model.geom('geom1').insertFile('petzval_lens_optimization_geom_sequence.mph', 'geom1');
...
model.param('par3').label('Parameters 3: Optimization');

% To import content from file, use:
% model.param('par3').loadFile('FILENAME');
model.param('par3').set('dR1_1', '0[mm]', 'L1, surface 1 adjustment');
model.param('par3').set('dR2_1', '0[mm]', 'L1, surface 2 adjustment');
model.param('par3').set('dR2_2', '0[mm]', 'L2, surface 2 adjustment');
model.param('par3').set('dR1_4', '0[mm]', 'L3, surface 1 adjustment');
model.param('par3').set('dR2_4', '0[mm]', 'L3, surface 2 adjustment');
model.param('par3').set('dR2_5', '0[mm]', 'L4, surface 2 adjustment');
model.param('par3').set('dR1_6', '0[mm]', ' L5, surface 1 adjustment');
model.param.set('R1_1', '99.56266[mm]+dR1_1');
model.param.descr('R1_1', 'L1, surface 1 radius of curvature');
model.param.set('R2_1', '-86.84002[mm]+dR2_1');
model.param.descr('R2_1', 'L1, surface 2 radius of curvature');
model.param.set('R2_2', '-1187.63858[mm]+dR2_2');
model.param.descr('R2_2', 'L2, surface 2 radius of curvature');
model.param.set('R1_4', '57.47191[mm]+dR1_4');
model.param.descr('R1_4', 'L3, surface 1 radius of curvature');
model.param.set('R2_4', '-54.61865[mm]+dR2_4');
model.param.descr('R2_4', 'L3, surface 2 radius of curvature');
model.param.set('R2_5', '-614.68633[mm]+dR2_5');
model.param.descr('R2_5', 'L4, surface 2 radius of curvature');
model.param.set('R1_6', '-38.17110[mm]+dR1_6');
model.param.descr('R1_6', 'L5, surface 1 radius of curvature');
...
model.study('std2').feature('opt').set('opttol', 0.001);
...
model.study('std2').feature('opt').setIndex('descr', '', 1);
model.study('std2').feature('opt').setIndex('optobj', 'comp1.gop.relg3.rrms^2', 2);
model.study('std2').feature('opt').setIndex('descr', '', 2);
model.study('std2').feature('opt').set('pname', {'dR1_1' 'dR2_1' 'dR2_2' 'dR1_4' 'dR2_4' 'dR2_5' 'dR1_6'});
model.study('std2').feature('opt').set('initval', {'0[mm]' '0[mm]' '0[mm]' '0[mm]' '0[mm]' '0[mm]' '0[mm]'});
model.study('std2').feature('opt').set('scale', {'0.01' '0.01' '1' '0.01' '0.01' '1' '0.01'});
model.study('std2').feature('opt').set('lbound', {'-20[mm]' '-20[mm]' '-500[mm]' '-10[mm]' '-10[mm]' '-500[mm]' '-10[mm]'});
model.study('std2').feature('opt').set('ubound', {'20[mm]' '20[mm]' '500[mm]' '10[mm]' '10[mm]' '500[mm]' '       10[mm]'});
...
model.title('Petzval Lens Optimization');

model.description('This example shows how to use an Optimization study to update the optical prescription for a multi-element objective lens when one of the glasses is replaced. Starting from the original prescription, the optical glass of one element was replaced with a glass of nearly the same d-line refractive index and Abbe number. In the Optimization study, perturbations in the radii of curvature of the lens surfaces were selected as control parameters, while the root mean square spot sizes for three different field angles were chosen as the objective functions.');
