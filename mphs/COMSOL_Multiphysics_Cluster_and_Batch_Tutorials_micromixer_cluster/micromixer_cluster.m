...
model.modelPath('/Applications/COMSOL62/Multiphysics/applications/COMSOL_Multiphysics/Cluster_and_Batch_Tutorials');
...
model.result('pg2').set('defaultPlotID', 'ResultDefaults_SinglePhaseFlow/icom1/pdef1/pcond2/pcond2/pcond4/pcond1/pg4');
...
model.study('std1').feature('cluco').set('clustertype', 'none');
model.study('std1').feature('cluco').set('batchdir', '/tmp/testusr/cscomsol37201/-ldh1ra9e159c');
...
model.batch('b1').set('batchdir', '/tmp/testusr/cscomsol37201/-ldh1ra9e159c');
...
model.batch('b1').set('specbatchdir', 'off');
model.batch('b1').set('rundir', '/home/testusr/Documents/COMSOL/Batch');
model.batch('b1').set('speccomsoldir', 'off');
model.batch('b1').set('comsoldir', '/hymer/build/main/daily');
model.batch('b1').set('control', 'cluco');
model.batch('c1').set('batch', 'b1');
model.batch('c1').set('clustertype', 'none');
model.batch('c1').set('hostfile', '');
model.batch('c1').set('nn', 1);
model.batch('c1').set('scheduler', 'localhost');
model.batch('c1').set('user', '');
model.batch('c1').set('control', 'cluco');
model.batch('c1').attach('std1');
...
model.batch('b1').set('batchfile', 'micromixer_cluster_np_1.mph');
model.batch('c1').run('compute');
model.batch('b1').feature('daDef').feature('pr2').set('operation', 'progress');
model.batch('b1').feature('daDef').feature('pr2').run;

model.title(['Micromixer ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' Cluster Version']);

model.description(['This version of the Micromixer model demonstrates how to use the cluster computing functionality in COMSOL Multiphysics and shows how to measure the speedup.' newline  newline 'This model requires a Floating Network License.']);
...
model.label('micromixer_cluster.mph');
