function out = model
%
% postbuckling_shell_arclength.m
%
...
model.cpl.remove('aveop1');

model.physics('shell').feature.remove('ge1');
...
model.sol('sol_in').study('std_in');
model.sol('sol_in').create('st1', 'StudyStep');
model.sol('sol_in').feature('st1').set('study', 'std_in');
model.sol('sol_in').feature('st1').set('studystep', 'stat_in');
model.sol('sol_in').create('v1', 'Variables');
model.sol('sol_in').feature('v1').feature('comp1_ar').set('scalemethod', 'manual');
model.sol('sol_in').feature('v1').feature('comp1_ar').set('resscalemethod', 'parent');
model.sol('sol_in').feature('v1').feature('comp1_ar').set('scaleval', '0.01');
model.sol('sol_in').feature('v1').set('control', 'stat_in');
model.sol('sol_in').create('s1', 'Stationary');
model.sol('sol_in').feature('s1').feature('aDef').set('cachepattern', true);
model.sol('sol_in').feature('s1').create('fc1', 'FullyCoupled');
model.sol('sol_in').feature('s1').feature('fc1').set('termonres', 'auto');
model.sol('sol_in').feature('s1').feature('fc1').set('reserrfact', 1000);
model.sol('sol_in').feature('s1').feature('fc1').set('linsolver', 'dDef');
model.sol('sol_in').feature('s1').feature('fc1').set('termonres', 'auto');
model.sol('sol_in').feature('s1').feature('fc1').set('reserrfact', 1000);
model.sol('sol_in').feature('s1').feature.remove('fcDef');
...
model.sol('sol_c').study('std_c');
model.sol('sol_c').create('st1', 'StudyStep');
model.sol('sol_c').feature('st1').set('study', 'std_c');
model.sol('sol_c').feature('st1').set('studystep', 'stat_c');
model.sol('sol_c').create('v1', 'Variables');
model.sol('sol_c').feature('v1').feature('comp1_ar').set('scalemethod', 'manual');
model.sol('sol_c').feature('v1').feature('comp1_ar').set('resscalemethod', 'parent');
model.sol('sol_c').feature('v1').feature('comp1_ar').set('scaleval', '0.01');
model.sol('sol_c').feature('v1').set('control', 'stat_c');
model.sol('sol_c').create('s1', 'Stationary');
model.sol('sol_c').feature('s1').feature('aDef').set('cachepattern', true);
model.sol('sol_c').feature('s1').create('fc1', 'FullyCoupled');
model.sol('sol_c').feature('s1').feature('fc1').set('termonres', 'auto');
model.sol('sol_c').feature('s1').feature('fc1').set('reserrfact', 1000);
model.sol('sol_c').feature('s1').feature('fc1').set('linsolver', 'dDef');
model.sol('sol_c').feature('s1').feature('fc1').set('termonres', 'auto');
model.sol('sol_c').feature('s1').feature('fc1').set('reserrfact', 1000);
model.sol('sol_c').feature('s1').feature.remove('fcDef');
...
model.sol('sol_p').study('std_p');
model.sol('sol_p').create('st1', 'StudyStep');
model.sol('sol_p').feature('st1').set('study', 'std_p');
model.sol('sol_p').feature('st1').set('studystep', 'stat_p');
model.sol('sol_p').create('v1', 'Variables');
model.sol('sol_p').feature('v1').feature('comp1_ar').set('scalemethod', 'manual');
model.sol('sol_p').feature('v1').feature('comp1_ar').set('resscalemethod', 'parent');
model.sol('sol_p').feature('v1').feature('comp1_ar').set('scaleval', '0.01');
model.sol('sol_p').feature('v1').set('control', 'stat_p');
model.sol('sol_p').create('s1', 'Stationary');
model.sol('sol_p').feature('s1').feature('aDef').set('cachepattern', true);
model.sol('sol_p').feature('s1').create('fc1', 'FullyCoupled');
model.sol('sol_p').feature('s1').feature('fc1').set('termonres', 'auto');
model.sol('sol_p').feature('s1').feature('fc1').set('reserrfact', 1000);
model.sol('sol_p').feature('s1').feature('fc1').set('linsolver', 'dDef');
model.sol('sol_p').feature('s1').feature('fc1').set('termonres', 'auto');
model.sol('sol_p').feature('s1').feature('fc1').set('reserrfact', 1000);
model.sol('sol_p').feature('s1').feature.remove('fcDef');
...
model.description(['For slender structures, buckling is a catastrophic instability if the service load is above the critical limit. For such structures, it can be important to study the behavior of the structure beyond the critical buckling load, which is known as postbuckling analysis. Tracing the equilibrium path in postbuckling analysis is not easy as it leads to numerical difficulties such as limit points. Using an arc length method is a well-known strategy to trace equilibrium paths in such situations.' newline  newline 'In this example, an incremental arc length method combined with cubic extrapolation is used for postbuckling analysis of a hinged cylindrical panel subjected to a point load at its center. The results of this example are compared with a similar example in which a global equation approach based on monotonically increasing displacement is used.']);
