...
model.physics('tff').feature('ffp1').set('rho', '860[kg/m^3]');
...
model.result('pg2').label('Velocity direction');
model.result('pg2').create('surf1', 'Surface');
model.result('pg2').feature('surf1').set('coloring', 'uniform');
model.result('pg2').feature('surf1').set('color', 'gray');
model.result('pg2').run;
model.result('pg2').create('arws1', 'ArrowSurface');
model.result('pg2').feature('arws1').set('expr', {'tff.vbx' 'tff.vby' 'tff.vbz'});
model.result('pg2').feature('arws1').set('descr', 'Velocity of base');
model.result('pg2').run;
model.result('pg2').set('edges', false);
model.result('pg2').set('titletype', 'none');
model.result.create('pg3', 'PlotGroup3D');
model.result('pg3').run;
model.result('pg3').label('Lubricant thickness');
model.result('pg3').create('surf1', 'Surface');
model.result('pg3').feature('surf1').set('expr', 'tff.h');
model.result('pg3').feature('surf1').set('descr', 'Total gap height');
...
model.title('Journal Bearing');

model.description('This example illustrates the calculation of the pressure distribution in a lubricant in a full journal bearing, where the bearing extends all around the rotating journal. In solving the Reynolds equation on the surface of the lubricant wrapped around the journal, the example uses a converging and diverging lubricant thickness to model the journal''s eccentric position within the bearing clearance.');
