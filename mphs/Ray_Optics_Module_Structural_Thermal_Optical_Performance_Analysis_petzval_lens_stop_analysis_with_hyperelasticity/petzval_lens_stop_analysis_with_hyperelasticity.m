function out = model
%
% petzval_lens_stop_analysis_with_hyperelasticity.m
%
...
model.physics('solid').feature('hmm1').selection.named('geom1_csel10_dom');
model.physics('solid').feature('hmm1').set('Compressibility_NeoHookean', 'Incompressible');
model.physics('solid').feature('hmm1').set('muLame_mat', 'userdef');
model.physics('solid').feature('hmm1').set('muLame', '40[MPa]');
model.physics('solid').feature('hmm1').create('te1', 'ThermalExpansion', 3);
...
model.result('pg6').feature('spot1').set('filterreleaseactive', true);
model.result.dataset('ip2').set('genmethod', 'threepoint');
model.result.dataset('ip2').setIndex('genpoints', '-2.603581892256138E-5[mm]', 0, 0);
model.result.dataset('ip2').setIndex('genpoints', '-1.3543118544169752E-6[mm]', 0, 1);
model.result.dataset('ip2').setIndex('genpoints', '162.79429854617553[mm]', 0, 2);
model.result.dataset('ip2').setIndex('genpoints', '-2.503581892256138E-5[mm]', 1, 0);
model.result.dataset('ip2').setIndex('genpoints', '-1.3543118544169752E-6[mm]', 1, 1);
model.result.dataset('ip2').setIndex('genpoints', '162.79429854617553[mm]', 1, 2);
model.result.dataset('ip2').setIndex('genpoints', '-2.603581892256138E-5[mm]', 2, 0);
model.result.dataset('ip2').setIndex('genpoints', '-3.543118544169752E-7[mm]', 2, 1);
model.result.dataset('ip2').setIndex('genpoints', '162.79429854617553[mm]', 2, 2);
model.result('pg6').set('ispendingzoom', true);
model.result('pg6').feature('spot1').run;
model.result('pg6').feature('spot1').set('filterreleaseactive', false);
model.result('pg6').run;

model.title('Petzval Lens STOP Analysis with Hyperelasticity');

model.description('This model demonstrates an integrated structural-thermal-optical performance (STOP) analysis of an optical system using a nonlinear material model. The Petzval Lens STOP analysis tutorial is used as the basis for this model. The lens supports are modeled using a hyperelastic material feature. The resulting displacement and stress fields within the optical system are shown together with nominal and best focus spot diagrams.');
