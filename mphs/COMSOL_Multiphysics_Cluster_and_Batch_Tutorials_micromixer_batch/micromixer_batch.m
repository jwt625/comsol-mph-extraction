...
model.study('std1').create('batch', 'Batch');
model.study('std1').feature('batch').set('clustersettings', true);
model.study('std1').feature('batch').set('synchsolutions', false);
model.study('std1').feature('batch').set('batchdir', '/tmp/testusr/cscomsol37201/-pe60dmryrdya');
...
model.batch('b1').set('batchdir', '/tmp/testusr/cscomsol37201/-pe60dmryrdya');
...
model.batch('b1').set('control', 'batch');
model.batch('b1').run('compute');
model.batch('b1').feature('daDef').run;
model.batch('b1').set('control', 'user');
model.batch('b1').set('npactive', true);
model.batch('b1').set('batchfile', 'batchmodel_np_1.mph');
model.batch('b1').run('compute');
model.batch('b1').feature('daDef').feature('pr2').set('operation', 'progress');
model.batch('b1').feature('daDef').feature('pr2').run;

model.title(['Micromixer ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' Batch Version']);

model.description('This version of the Micromixer model demonstrates how to use the batch functionality in COMSOL Multiphysics and shows how to measure the speedup.');
...
model.label('micromixer_batch.mph');
