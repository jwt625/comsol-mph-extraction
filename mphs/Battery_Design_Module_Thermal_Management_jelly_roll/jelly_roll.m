...
model.geom('geom1').insertFile('jelly_roll_geom_sequence.mph', 'geom1');
...
model.param('par2').set('H_mesh', 'H_jr/10/3', 'Maximum mesh size (in xy-plane)');
...
model.material('mat6').selection.named('geom1_ext4_dom');
...
model.physics('cd').feature('cc1').selection.named('geom1_unisel4');
model.physics('cd').create('pce1', 'PorousElectrode', 3);
model.physics('cd').feature('pce1').selection.named('geom1_unisel5');
...
model.physics('cd').feature('egnd1').selection.named('geom1_sel6');
model.physics('cd').create('ec1', 'ElectrodeCurrent', 2);
model.physics('cd').feature('ec1').selection.named('geom1_sel7');
...
model.mesh('mesh1').feature('size1').selection.named('geom1_unisel6');
...
model.mesh('mesh1').feature('fq1').selection.named('geom1_sel9');
model.mesh('mesh1').run('fq1');
model.mesh('mesh1').create('swe1', 'Sweep');
model.mesh('mesh1').feature('swe1').create('dis1', 'Distribution');
model.mesh('mesh1').feature('swe1').feature('dis1').selection.named('geom1_sel1');
model.mesh('mesh1').feature('swe1').feature('dis1').set('numelem', 2);
model.mesh('mesh1').feature('swe1').create('dis2', 'Distribution');
model.mesh('mesh1').feature('swe1').feature('dis2').selection.named('geom1_unisel5');
model.mesh('mesh1').feature('swe1').create('dis3', 'Distribution');
model.mesh('mesh1').feature('swe1').feature('dis3').selection.named('geom1_unisel4');
...
model.result('pg1').label('Electrode Potential wrt Negative Terminal');
...
model.result('pg1').feature('vol1').feature('sel1').selection.named('geom1_unisel3');
model.result('pg1').run;
...
model.result('pg2').label('Electrode Potential wrt Positive Terminal');
model.result('pg2').run;
model.result('pg2').feature('vol1').set('descr', 'Electric potential Electric potential on boundary');
model.result('pg2').feature('vol1').set('expr', 'cd.phis-cd.phis0_ec1');
model.result('pg2').feature('vol1').set('unit', 'mV');
model.result('pg2').run;
model.result('pg2').feature('vol1').feature('sel1').selection.named('geom1_unisel2');
model.result('pg2').run;
model.result('pg2').run;
...
model.physics('ht').feature('hf1').selection.named('geom1_sel8');
...
model.result('pg3').feature('vol1').set('descr', 'Temperature');
...
model.title('Jelly Roll');

model.description(['In a cylindrical or prismatic battery cell, the active layers, current collector metal foils and separators are wound into a "jelly roll".' newline  newline 'Additional tabs (metal strips) are welded to the current collector foils in order to conduct the current to the exterior of the cell can.' newline  newline 'The interplay of the various dimensions of the layers and the tabs, in combination with the magnitude of the cell current, governs the temperature and current distribution in the battery cell.' newline  newline 'This tutorial models the ohmic and activation losses, and the resulting temperature distribution, in a jelly roll for a pseudo-stationary case at a constant cell current.']);
