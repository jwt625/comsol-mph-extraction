...
model.physics('solid').feature('rig1').set('xc', {'0' '-0.40' '-0.10'});
...
model.physics('solid').feature('rig1').set('Omega', [0 1 0]);
model.physics('solid').feature('rig1').set('phi0', '0.05[deg]');
model.physics('solid').feature('rig1').create('rf1', 'RigidBodyForce', -1);
model.physics('solid').feature('rig1').feature('rf1').set('Ft', {'0' '-10[kN]' '0'});
...
model.title(['Bracket ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' Rigid Connector Analysis']);

model.description('This example shows how to model a rigid structure connected to an elastic part.');

model.mesh.clearMeshes;

model.sol('sol1').clearSolutionData;

model.label('bracket_rigid_connector.mph');
