...
model.geom('geom1').feature('i1').setIndex('coord', 1000, 2);
...
model.param.set('V0', '-2[V]', 'Initial voltage');
model.param.set('dVdt', '1[V/s]', 'Voltage ramp rate');
model.param.set('V1', '3[V]', 'Final voltage');
model.param.set('t', '0[s]', 'Time parameter');
model.param.set('tmax', 'V1/dVdt', 'Final time');
model.param.set('dt', 'tmax/50', 'Time step');
...
model.physics('semi').feature('gc1').set('V0', 'V0+dVdt*t');
...
model.physics('semi').feature('ge1').setIndex('name', 'Q1', 0, 0);
model.physics('semi').feature('ge1').setIndex('equation', 'Q1t-semi.I0_1', 0, 0);
model.physics('semi').feature('ge1').set('CustomDependentVariableUnit', '1');
model.physics('semi').feature('ge1').set('DependentVariableQuantity', 'none');
model.physics('semi').feature('ge1').setIndex('CustomDependentVariableUnit', 'C', 0, 0);
model.physics('semi').feature('ge1').set('CustomSourceTermUnit', '1');
model.physics('semi').feature('ge1').set('SourceTermQuantity', 'none');
model.physics('semi').feature('ge1').setIndex('CustomSourceTermUnit', 'A', 0, 0);
...
model.mesh('mesh1').feature('edg1').feature('dis2').set('elemratio', 5000);
model.mesh('mesh1').feature('edg1').feature('dis2').set('growthrate', 'exponential');

model.study('std1').create('param', 'Parametric');
model.study('std1').feature('param').setIndex('pname', 'area', 0);
...
model.study('std1').feature('param').setIndex('plistarr', '1e-3 1e3', 0);
model.study('std1').feature('param').setIndex('punit', 'V/s', 0);
model.study('std1').feature('stat').set('useadvanceddisable', true);
model.study('std1').feature('stat').set('disabledphysics', {'semi/ge1'});
model.study('std1').create('time', 'Transient');
model.study('std1').feature('time').set('tlist', 'range(0,dt,tmax)');
...
model.sol('sol1').feature('t1').set('tlist', 'range(0,dt,tmax)');
...
model.batch('p1').set('pname', {'dVdt'});
model.batch('p1').set('plistarr', {'1e-3 1e3'});
...
model.result('pg4').setIndex('looplevelindices', 'range(2,1,51)', 0);
model.result('pg4').create('glob1', 'Global');
model.result('pg4').feature('glob1').set('markerpos', 'datapoints');
model.result('pg4').feature('glob1').set('linewidth', 'preference');
model.result('pg4').feature('glob1').setIndex('expr', 'd(semi.Q0_2,t)/dVdt/C0', 0);
model.result('pg4').feature('glob1').setIndex('descr', 'Normalized capacitance', 0);
model.result('pg4').feature('glob1').set('xdata', 'expr');
model.result('pg4').feature('glob1').set('xdataexpr', 'semi.V0_2');
...
model.result('pg4').feature('glob2').setIndex('looplevelinput', 'manualindices', 0);
model.result('pg4').feature('glob2').setIndex('looplevelindices', 'range(2,51)', 0);
model.result('pg4').feature('glob2').setIndex('expr', '-(Q1-withsol(''sol3'',Q1,setval(dVdt,dVdt),setval(t,t-dt)))/dt/dVdt/C0', 0);
model.result('pg4').feature('glob2').set('xdataexpr', 'semi.V0_2-0.5*dVdt*dt');
model.result('pg4').feature('glob2').set('xdatadescractive', true);
model.result('pg4').feature('glob2').set('xdatadescr', 'Terminal voltage');
model.result('pg4').feature('glob2').set('linestyle', 'dotted');
model.result('pg4').feature('glob2').set('linewidth', 2);
model.result('pg4').run;

model.title('MOSCAP 1D');

model.description('The metal-silicon-oxide (MOS) structure is the fundamental building block for many silicon planar devices. Its capacitance measurements provide a wealth of insight into the working principles of such devices. This tutorial constructs a simple 1D model of a MOS capacitor (MOSCAP). Both the low- and high-frequency C-V curves are computed using the transient study approach (the related model moscap_1d_small_signal shows the small-signal approach). Two alternative methods to compute the differential capacitance are demonstrated, using either the gate terminal or the metal contact terminal, respectively.');
