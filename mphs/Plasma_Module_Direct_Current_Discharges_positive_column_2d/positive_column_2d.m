...
model.geom('geom1').feature('r1').set('size', [0.05 0.4]);
model.geom('geom1').run('r1');
model.geom('geom1').create('r2', 'Rectangle');
model.geom('geom1').feature('r2').set('size', {'0.0375' '6e-3'});
model.geom('geom1').feature('r2').set('pos', [0 0.01]);
model.geom('geom1').run('r2');
model.geom('geom1').create('r3', 'Rectangle');
model.geom('geom1').feature('r3').set('size', {'0.0375' '6e-3'});
model.geom('geom1').feature('r3').set('pos', [0 0.384]);
model.geom('geom1').run('r3');
model.geom('geom1').create('co1', 'Compose');
model.geom('geom1').feature('co1').selection('input').set({'r1'});
model.geom('geom1').feature('co1').set('formula', 'r1-r2-r3');
...
model.geom('geom1').feature('ls1').set('coord1', [0 0.02]);
model.geom('geom1').feature('ls1').set('coord2', [0.0375 0.02]);
...
model.variable('var1').set('mueN', '1E25[1/(m*V*s)]');
model.variable('var1').descr('mueN', 'Reduced electron mobility');
model.variable('var1').set('V0', '125[V]');
model.variable('var1').descr('V0', 'Applied voltage');
model.variable('var1').set('Wf', '5');
model.variable('var1').descr('Wf', 'Work function');
model.variable('var1').set('p0', '0.5[torr]');
...
model.selection('sel1').set([3 5 10]);
model.selection.create('sel2', 'Explicit');
model.selection('sel2').model('comp1');
model.selection('sel2').label('Anode');
model.selection('sel2').geom(1);
model.selection('sel2').set([6 8 11]);
model.selection.create('sel3', 'Explicit');
model.selection('sel3').model('comp1');
model.selection('sel3').label('Walls');
model.selection('sel3').geom(1);
model.selection('sel3').set([2 9 12]);
model.selection.create('sel4', 'Explicit');
model.selection('sel4').model('comp1');
model.selection('sel4').label('All Walls');
model.selection('sel4').geom(1);
model.selection('sel4').set([2 3 5 6 8 9 10 11 12]);
model.selection.create('sel5', 'Explicit');
model.selection('sel5').model('comp1');
model.selection('sel5').label('Noncathode Walls');
model.selection('sel5').geom(1);
model.selection('sel5').set([2 6 8 9 11 12]);
...
model.physics('plas').feature('wall1').selection.named('sel4');
...
model.physics('plas').feature('mct1').selection.named('sel2');
model.physics('plas').feature('mct1').set('V0', 'V0');
model.physics('plas').feature('mct1').set('CircuitType', 'SeriesRCCircuit');
model.physics('plas').create('dct1', 'DielectricContact', 1);
model.physics('plas').feature('dct1').selection.named('sel3');
...
model.physics('plas').feature('sr1').set('gammai', 0.25);
...
model.physics('plas').feature('sr2').selection.named('sel5');
model.physics('plas').create('sr3', 'SurfaceReaction', 1);
model.physics('plas').feature('sr3').set('formula', 'Ars=>Ar');
model.physics('plas').feature('sr3').selection.named('sel4');

model.mesh('mesh1').create('edg1', 'Edge');
model.mesh('mesh1').feature('edg1').selection.set([3 5 6 8 10 11 14]);
...
model.mesh('mesh1').feature('bl1').feature('blp').selection.named('sel4');
...
model.sol('sol1').feature('t1').set('tlist', '0 10^{range(-8,0.4,0)}');
...
model.result('pg4').label('Electric Potential on Axis');
model.result('pg4').setIndex('looplevelinput', 'last', 0);
model.result('pg4').set('xlabelactive', true);
model.result('pg4').set('xlabel', 'Distance (x)');
...
model.result('pg4').feature('lngr1').set('descr', 'Electric potential');
model.result('pg4').run;
model.result('pg4').run;
model.result.create('pg5', 'PlotGroup1D');
model.result('pg5').run;
model.result('pg5').label('Electron Temperature on Axis');
model.result('pg5').setIndex('looplevelinput', 'last', 0);
model.result('pg5').set('xlabelactive', true);
model.result('pg5').set('xlabel', 'Distance (x)');
model.result('pg5').set('ylabelactive', true);
model.result('pg5').set('ylabel', 'Electron Temperature (V)');
...
model.result('pg6').label('Electron Density on Axis');
model.result('pg6').setIndex('looplevelinput', 'last', 0);
model.result('pg6').set('xlabelactive', true);
model.result('pg6').set('xlabel', 'Distance (x)');
model.result('pg6').set('ylabelactive', true);
model.result('pg6').set('ylabel', 'Electron Density (1/m<sup>3</sup>)');
...
model.result('pg7').label('Excited Argon Number Density on Axis');
model.result('pg7').setIndex('looplevelinput', 'last', 0);
model.result('pg7').set('xlabelactive', true);
model.result('pg7').set('xlabel', 'Distance (x)');
model.result('pg7').set('ylabelactive', true);
model.result('pg7').set('ylabel', 'Excited Argon Number Density (1/m<sup>3</sup>)');
...
model.result('pg8').label('Argon Ion Number Density on Axis');
model.result('pg8').setIndex('looplevelinput', 'last', 0);
model.result('pg8').set('xlabelactive', true);
model.result('pg8').set('xlabel', 'Distance (x)');
model.result('pg8').set('ylabelactive', true);
model.result('pg8').set('ylabel', 'Argon Ion Number Density (1/m<sup>3</sup>)');
...
model.title('DC Glow Discharge');

model.description('DC glow discharges in the low-pressure regime have long been used for gas lasers and fluorescent lamps. DC discharges are attractive to study because the solution is time independent. This 2D model shows how to use the DC Discharge interface to set up an analysis of a positive column. The discharge is sustained by emission of secondary electrons at the cathode.');
