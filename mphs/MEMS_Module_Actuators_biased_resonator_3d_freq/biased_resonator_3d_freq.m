...
model.param.set('Q', '8000');
model.param.descr('Q', 'Resonator quality factor');
model.param.set('f0', '8[MHz]');
model.param.descr('f0', 'Approximate resonance frequency');
model.param.set('alpha', '4*pi*f0/(3*Q)');
model.param.descr('alpha', 'Damping parameter');
model.param.set('beta', '1/(6*pi*f0*Q)');
model.param.descr('beta', 'Damping parameter');

model.physics('solid').feature('lemm1').create('dmp1', 'Damping', 3);
model.physics('solid').feature('lemm1').feature('dmp1').set('alpha_dM', 'alpha');
model.physics('solid').feature('lemm1').feature('dmp1').set('beta_dK', 'beta');
model.physics('es').feature('term2').create('hp1', 'HarmonicPerturbation', 3);
model.physics('es').feature('term2').feature('hp1').set('V0', '3[mV]');
...
model.study('std2').feature('frlin').setSolveFor('/physics/es', true);
model.study('std2').feature('frlin').setSolveFor('/multiphysics/eme1', true);
model.study('std2').feature('frlin').set('punit', 'MHz');
model.study('std2').feature('frlin').set('plist', 'range(7.69,0.001,7.73)');
model.study('std2').label('Frequency Domain');
...
model.sol('sol2').feature('v1').feature('comp1_spatial_disp').set('scaleval', '1e-2*4.097926304852248E-5');
model.sol('sol2').feature('v1').feature('comp1_u').set('scaleval', '1e-2*4.097926304852248E-5');
...
model.sol('sol2').feature('st2').set('studystep', 'frlin');
model.sol('sol2').create('v2', 'Variables');
model.sol('sol2').feature('v2').feature('comp1_spatial_disp').set('scalemethod', 'manual');
model.sol('sol2').feature('v2').feature('comp1_spatial_disp').set('scaleval', '3.4883509312204626E-7');
...
model.sol('sol2').feature('v2').set('control', 'frlin');
...
model.sol('sol2').feature('s2').feature('p1').set('plistarr', {'range(7.69,0.001,7.73)'});
model.sol('sol2').feature('s2').feature('p1').set('punit', {'MHz'});
...
model.sol('sol2').feature('s2').feature('p1').set('plot', 'off');
model.sol('sol2').feature('s2').feature('p1').set('plotgroup', 'Default');
model.sol('sol2').feature('s2').feature('p1').set('probesel', 'all');
model.sol('sol2').feature('s2').feature('p1').set('probes', {});
model.sol('sol2').feature('s2').feature('p1').set('control', 'frlin');
model.sol('sol2').feature('s2').set('nonlin', 'linper');
model.sol('sol2').feature('s2').set('storelinpoint', true);
model.sol('sol2').feature('s2').set('linpsolnum', 'all');
model.sol('sol2').feature('s2').set('control', 'frlin');
model.sol('sol2').feature('s2').set('linpmethod', 'sol');
model.sol('sol2').feature('s2').set('linpsol', 'sol2');
model.sol('sol2').feature('s2').set('linpsoluse', 'sol3');
model.sol('sol2').feature('s2').set('control', 'frlin');
...
model.result('pg5').feature('ptgr1').selection.set([254]);
model.result('pg5').feature('ptgr1').set('expr', 'abs(w)');
model.result('pg5').run;
model.result('pg5').set('ylog', true);
model.result('pg5').run;
model.result('pg5').label('Frequency Domain');

model.title(['Frequency Response of a Biased Resonator ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' 3D']);

model.description('An electrostatically actuated MEMS resonator is simulated. The device is driven by an AC + DC bias voltage applied across a parallel plate capacitor. In this example, the frequency response of the biased resonator is computed.');
...
model.label('biased_resonator_3d_freq.mph');
