function out = model
%
% dielectric_shielding_comparison.m
%
...
model.param.descr('er_a', 'Relative permittivity, free space');
...
model.geom('geom1').feature('mir1').selection('input').set({'copy1' 'fil1' 'r1'});
...
model.selection('sel1').set([4 5]);
model.selection('sel1').label('Full fidelity');
model.selection.create('sel2', 'Explicit');
model.selection('sel2').model('comp1');
model.selection('sel2').geom(1);
model.selection('sel2').set([42 43]);
model.selection('sel2').label('Dielectric shielding');
model.selection.create('sel3', 'Explicit');
model.selection('sel3').model('comp1');
model.selection('sel3').set([1 4 5 6 7 9]);
model.selection('sel3').label('Model domain');

model.physics('es').selection.named('sel3');
model.physics('es').create('gnd1', 'Ground', 1);
model.physics('es').feature('gnd1').selection.set([22 24 44 45]);
model.physics('es').create('gnd2', 'Ground', 1);
model.physics('es').feature('gnd2').selection.set([4 5 34 35]);
model.physics('es').create('term1', 'Terminal', 1);
model.physics('es').feature('term1').selection.set([29 31 46 47]);
...
model.physics('es').feature('term2').selection.set([6 7 36 37]);
model.physics('es').feature('term2').set('TerminalType', 'Voltage');
model.physics('es').feature('term2').set('V0', 'V0');
model.physics('es').create('des1', 'DielectricShielding', 1);
model.physics('es').feature('des1').selection.named('sel2');
model.physics('es').feature('des1').set('ds', '1[mm]');
model.physics('es').create('symp1', 'SymmetryPlane', 1);
model.physics('es').feature('symp1').selection.set([9 11 13 14 16 18 19 23 25 26 27 30]);
...
model.material('mat3').propertyGroup('def').set('relpermittivity', {'er_b'});
...
model.title('Dielectric Shielding Comparison');

model.description('Available for electrostatic field modeling, the dielectric shielding boundary condition, approximates a thin layer of material with high relative permittivity compared to its surroundings. This example compares the dielectric shielding boundary condition to a full-fidelity model and discusses the range of applicability of this boundary condition.');
