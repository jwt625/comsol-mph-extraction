...
model.variable('var1').set('epsl', 'max(min(phils,1),if(y<3e-5[m],1e-9,1))', 'Electrolyte volume fraction');
...
model.physics('tcd').feature('hcpce1').feature('per1').set('Av', 'ls.delta');
...
model.physics('ls').feature('inl1').selection.set([2 4 5 6 7 9 10 11 12]);
model.physics('ls').create('out1', 'Outlet', 1);
model.physics('ls').feature('out1').selection.set([1 3 8]);
...
model.sol('sol1').feature('t1').set('atolmethod', {'comp1_cSO4' 'global' 'comp1_GI' 'global' 'comp1_phil' 'global' 'comp1_phils' 'scaled' 'comp1_tcd_es1_c' 'global'});
model.sol('sol1').feature('t1').set('atolvaluemethod', {'comp1_cSO4' 'factor' 'comp1_GI' 'factor' 'comp1_phil' 'factor' 'comp1_phils' 'factor' 'comp1_tcd_es1_c' 'factor'});
model.sol('sol1').feature('t1').set('atolfactor', {'comp1_cSO4' '0.1' 'comp1_GI' '0.1' 'comp1_phil' '0.1' 'comp1_phils' '0.01' 'comp1_tcd_es1_c' '0.1'});
...
model.sol('sol1').feature('t1').feature('d1').label('Direct, level set variable (ls) (Merged)');
...
model.title('Copper Deposition in a Trench Using the Level Set Method');

model.description('This example illustrates use of the Level Set interface to model electrodeposition of copper that takes place on a cathode surface. The model captures a cavity formation which occurs due to nonuniform deposition rate. Both concentration dependent electrode kinetics as well as ion transport by diffusion and migration are accounted for, and the model is therefore referred to as a tertiary current density distribution model. The electrode kinetics at the deposition boundary is defined as a domain term making use of the delta function from the Level Set interface.');
