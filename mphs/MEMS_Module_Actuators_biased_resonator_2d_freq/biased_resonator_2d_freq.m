...
model.param.set('alpha', '4189[Hz]');
model.param.descr('alpha', 'Damping parameter - alpha');
model.param.set('beta', '8.29e-13[s]');
model.param.descr('beta', 'Damping parameter - beta');
...
model.physics('es').feature('pot1').create('hp1', 'HarmonicPerturbation', 1);
model.physics('es').feature('pot1').feature('hp1').set('V0', 0.1);
...
model.study('std2').feature('frlin').set('plist', 'range(7.95[MHz],0.01[MHz],8.25[MHz])');
...
model.sol('sol2').feature('v2').feature('comp1_spatial_disp').set('scaleval', '2.6454999527499523E-7');
...
model.sol('sol2').feature('s2').feature('p1').set('plistarr', {'range(7.95[MHz],0.01[MHz],8.25[MHz])'});
...
model.result('pg4').label('Frequency Domain');
model.result('pg4').create('ptgr1', 'PointGraph');
model.result('pg4').feature('ptgr1').set('markerpos', 'datapoints');
model.result('pg4').feature('ptgr1').set('linewidth', 'preference');
model.result('pg4').feature('ptgr1').selection.set([9]);
model.result('pg4').feature('ptgr1').set('expr', 'v');
model.result('pg4').run;

model.title(['Frequency Response of a Biased Resonator ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' 2D']);

model.description('An electrostatically actuated MEMS resonator is simulated. The device is driven by an AC + DC bias voltage applied across a parallel plate capacitor. In this example, frequency response of the biased resonator is computed.');
...
model.label('biased_resonator_2d_freq.mph');
