function out = model
%
% thermal_actuator_jh_distributed.m
%
...
model.result('pg3').feature('vol1').label('Volume');
...
model.study('std1').feature('cluco').set('batchdir', '/tmp/testusr/cscomsol37201/-nlh2zf1tu8xo');
...
model.sol('sol1').feature('s1').feature('p1').set('pname', {'DV'});
model.sol('sol1').feature('s1').feature('p1').set('plistarr', {'range(5,0.5,15)'});
...
model.batch('b1').set('batchdir', '/tmp/testusr/cscomsol37201/-nlh2zf1tu8xo');
...
model.title(['Joule Heating of a Microactuator ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' Distributed Parameter Version']);

model.description(['This version of the Joule Heating of a Microactuator model demonstrates how to take advantage of the distributed parameter functionality in COMSOL Multiphysics and shows how to measure the speedup.' newline  newline 'The example requires a Floating Network License.']);
