...
model.param.set('th', '0.06[cm]', 'Thickness');
...
model.variable('var1').set('wmax_ref', '21.75[cm]');
model.variable('var1').descr('wmax_ref', 'Transverse displacement at midpoint, reference');
model.variable('var1').set('vmax_ref', '12.03[cm]');
model.variable('var1').descr('vmax_ref', 'Lateral displacement at midpoint on side edge, reference');
model.variable('var1').set('dispmax_ref', '4.94[cm]');
model.variable('var1').descr('dispmax_ref', 'Total displacement at corner point, reference');
model.variable('var1').set('sp1max_ref', '0.39[kN/cm^2]');
model.variable('var1').descr('sp1max_ref', 'First principal stress at midpoint, reference');
...
model.sol('sol1').feature('s1').feature('p1').set('pmaxstep', 0.01);
...
model.result('pg2').feature('surf1').set('unit', 'kN/cm^2');
...
model.result('pg5').feature('ptgr2').setIndex('legends', 'Lateral displacement at midpoint on side edge', 0);
model.result('pg5').feature.duplicate('ptgr3', 'ptgr2');
model.result('pg5').run;
model.result('pg5').feature('ptgr3').selection.set([3]);
model.result('pg5').feature('ptgr3').set('expr', 'mbrn.disp');
model.result('pg5').feature('ptgr3').setIndex('legends', 'Total displacement at corner point', 0);
model.result('pg5').run;
...
model.result('pg5').feature('glob1').set('linecolor', 'fromtheme');
...
model.result('pg5').run;
model.result('pg5').feature('glob2').setIndex('expr', 'vmax_ref', 0);
model.result('pg5').feature('glob2').setIndex('unit', 'cm', 0);
model.result('pg5').feature('glob2').setIndex('descr', 'Lateral displacement at midpoint on side edge, reference', 0);
model.result('pg5').feature('glob2').set('legend', false);
model.result('pg5').feature.duplicate('glob3', 'glob2');
model.result('pg5').run;
model.result('pg5').feature('glob3').setIndex('expr', 'dispmax_ref', 0);
model.result('pg5').feature('glob3').setIndex('unit', 'cm', 0);
model.result('pg5').feature('glob3').setIndex('descr', 'Total displacement at corner point, reference', 0);
model.result('pg5').run;
...
model.result('pg6').feature('glob1').setIndex('looplevelinput', 'last', 0);
model.result('pg6').feature('glob1').setIndex('expr', 'sp1max_ref', 0);
model.result('pg6').feature('glob1').setIndex('unit', 'kN/cm^2', 0);
model.result('pg6').feature('glob1').setIndex('descr', 'First principal stress at midpoint, reference', 0);
model.result('pg6').feature('glob1').set('titletype', 'none');
model.result('pg6').feature('glob1').set('linecolor', 'fromtheme');
model.result('pg6').feature('glob1').set('linemarker', 'square');
model.result('pg6').feature('glob1').set('markerpos', 'interp');
model.result('pg6').feature('glob1').set('legendmethod', 'manual');
model.result('pg6').feature('glob1').setIndex('legends', 'Reference value at last step', 0);
model.result('pg6').run;
model.result('pg6').run;
model.result('pg1').run;
model.result('pg4').run;

model.title('Inflation of a Square Airbag');

model.description('A square airbag is inflated using a constant air pressure. In some regions of the airbag the material wrinkles during inflation, as a negative principal stress develops. The wrinkling membrane model within the tension field theory gives a correct description of the stress distribution and wrinkling pattern in inflated membranes.');

model.mesh.clearMeshes;

model.sol('sol1').clearSolutionData;

model.label('membrane_airbag_inflation.mph');
