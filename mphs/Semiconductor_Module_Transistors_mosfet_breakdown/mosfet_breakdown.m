...
model.param.set('Vg', '3[V]');
model.param.set('Vbreak', '11.4[V]');
model.param.descr('Vbreak', 'Breakdown voltage');

model.func.create('rm1', 'Ramp');
model.func('rm1').model('comp1');
model.func('rm1').set('slope', 1000);
model.func('rm1').set('location', '5e-5');
...
model.func('rm1').set('smoothzoneloc', '0.0001');
model.func('rm1').set('smoothzonecutoff', '0.0001');

model.variable.create('var1');
model.variable('var1').model('comp1');
model.variable('var1').set('Vds', 'Vbreak*(rm1(t[1/s]))');

model.physics('semi').create('mc4', 'MetalContact', 1);
model.physics('semi').feature('mc4').selection.set([7]);
model.physics('semi').feature('mc4').set('V0', 'Vd');
model.physics('semi').feature('mc2').set('V0', 'Vds');
model.physics('semi').create('iig1', 'IIGeneration', 2);
...
model.study('std3').feature('stat').set('disabledphysics', {'semi/mc2' 'semi/iig1'});
model.study('std3').create('time', 'Transient');
model.study('std3').feature('time').set('tlist', 'range(0,0.2/15,0.18)*1e-3 range(0.2,0.6/5,0.8)*1e-3 range(0.85,0.15/5,1)*1e-3');
model.study('std3').feature('time').set('useadvanceddisable', true);
model.study('std3').feature('time').set('disabledphysics', {'semi/mc4'});
...
model.sol('sol3').feature('t1').set('tlist', 'range(0,0.2/15,0.18)*1e-3 range(0.2,0.6/5,0.8)*1e-3 range(0.85,0.15/5,1)*1e-3');
...
model.sol('sol3').feature('t1').set('initialstepbdf', '5e-6');
model.sol('sol3').feature('t1').set('atolglobalvaluemethod', 'manual');
model.sol('sol3').feature('t1').set('atolglobal', '1e-6');
...
model.sol('sol3').feature('t1').feature('st1').setIndex('stopcondActive', true, 0);
model.sol('sol3').feature('t1').feature('st1').setIndex('stopconddesc', 'Stop expression 1', 0);
model.sol('sol3').feature('t1').feature('st1').setIndex('stopcondarr', 'root.comp1.semi.I0_2>200e-6', 0);
...
model.result('pg8').set('xlabel', 'Drain Voltage, Vds (V)');
model.result('pg8').set('ylabelactive', true);
model.result('pg8').set('ylabel', 'Terminal current (uA)');
model.result('pg8').set('titletype', 'none');
model.result('pg8').set('legendpos', 'upperleft');
model.result('pg8').label('Id vs. Vd with impact ionization');
model.result('pg8').create('glob1', 'Global');
model.result('pg8').feature('glob1').set('markerpos', 'datapoints');
model.result('pg8').feature('glob1').set('linewidth', 'preference');
model.result('pg8').feature('glob1').set('expr', {'semi.I0_2'});
model.result('pg8').feature('glob1').set('descr', {'Terminal current'});
model.result('pg8').feature('glob1').set('unit', {'A'});
model.result('pg8').feature('glob1').set('xdata', 'expr');
model.result('pg8').feature('glob1').set('xdataexpr', 'Vds');
model.result('pg8').feature('glob1').setIndex('unit', 'uA', 0);
model.result('pg8').feature('glob1').setIndex('descr', 'Terminal current', 0);
model.result('pg8').feature('glob1').set('legendmethod', 'manual');
model.result('pg8').feature('glob1').setIndex('legends', 'Vg = 3 V', 0);
model.result('pg8').feature('glob1').set('linemarker', 'diamond');
model.result('pg8').run;
model.result('pg5').run;
model.result.duplicate('pg9', 'pg5');
model.result('pg9').run;
model.result('pg9').label('Impact Ionization');
model.result('pg9').set('data', 'dset3');
model.result('pg9').set('titletype', 'none');
model.result('pg9').run;
model.result('pg9').feature('surf1').set('expr', 'semi.Gii');
model.result('pg9').run;
model.result('pg9').run;
model.result.duplicate('pg10', 'pg9');
model.result('pg10').run;
model.result('pg9').run;
model.result('pg9').label('Impact Ionization log scale');
model.result('pg9').run;
model.result('pg9').feature('surf1').set('expr', 'log10(-semi.Gii+eps)');
model.result('pg9').feature('surf1').set('rangecoloractive', true);
model.result('pg9').feature('surf1').set('rangecolormax', 36);
model.result('pg9').run;
model.result('pg9').run;

model.title('Breakdown in a MOSFET');

model.description('This example shows how to use the time dependent solver to model the breakdown of a MOSFET as a result of impact ionization. MOSFETs typically operate in three regimes depending on the drain-source voltage for a given gate voltage. Initially the current-voltage relation is linear, this is the ohmic region. As the drain-source voltage increases the extracted current begins to saturate, this is the saturation region. As the drain-source voltage is further increased the breakdown region is entered, where the current increases exponentially for a small increase in the applied voltage. This is due to impact ionization.');
...
model.label('mosfet_breakdown.mph');
