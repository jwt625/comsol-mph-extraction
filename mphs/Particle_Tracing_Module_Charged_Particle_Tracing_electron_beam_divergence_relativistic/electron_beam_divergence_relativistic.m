function out = model
%
% electron_beam_divergence_relativistic.m
%
...
model.physics('cpt').prop('RelativisticCorrection').set('RelativisticCorrection', '1');
...
model.physics('cpt').create('mf1', 'MagneticForce');
...
model.multiphysics.create('mpfi1', 'MagneticParticleFieldInteraction', 'geom1', 3);
model.multiphysics('mpfi1').set('CurrentDensitySource_physics', 'cpt');
model.multiphysics('mpfi1').set('CurrentDensityDestination_physics', 'mf');
...
model.param.set('Ibeam', '3[A]', 'Beam current magnitude');
model.param.set('v0beam', '1e8[m/s]', 'Initial beam velocity');
model.param.set('gamma', '1/sqrt(1-(v0beam/c_const)^2)', 'Relativistic factor');
model.param.set('K', 'e_const*Ibeam/(2*pi*epsilon0_const*me_const*(v0beam*gamma)^3)', 'Generalized perveance');
model.param.set('hmax', '0.01', 'Maximum mesh element size');
...
model.physics('cpt').feature('mf1').set('UsePPR', true);
...
model.sol('sol1').feature('v2').feature('comp1_A').set('solvefor', 'on');
model.sol('sol1').feature('v2').feature('comp1_epfi1_rhos_accum').set('solvefor', 'off');
model.sol('sol1').feature('v2').feature('comp1_mpfi1_js').set('solvefor', 'off');
...
model.sol('sol1').feature('s1').feature('i2').feature('mg1').feature('cs').create('ams1', 'AMS');
model.sol('sol1').feature('s1').feature('i2').feature('mg1').feature('cs').feature('ams1').set('prefun', 'ams');
model.sol('sol1').feature('s1').feature('i2').feature('mg1').feature('cs').feature('ams1').set('sorvecdof', {'comp1_A'});
...
model.sol('sol1').feature('v3').feature('comp1_A').set('solvefor', 'off');
model.sol('sol1').feature('v3').feature('comp1_epfi1_rhos_accum').set('solvefor', 'off');
model.sol('sol1').feature('v3').feature('comp1_mpfi1_js').set('solvefor', 'off');
...
model.sol('sol1').feature('v4').feature('comp1_A').set('solvefor', 'off');
model.sol('sol1').feature('v4').feature('comp1_epfi1_rhos_accum').set('solvefor', 'on');
model.sol('sol1').feature('v4').feature('comp1_mpfi1_js').set('solvefor', 'on');
...
model.sol('sol1').feature('t1').set('tlist', 'range(0,1.0e-10,3e-9)');
...
model.sol('sol1').feature('t1').set('initialstepgenalpha', '(1.0E-13)[s]');
...
model.sol('sol1').feature('t1').feature('se1').feature('ss3').set('segvar', {'comp1_mpfi1_js'});
model.sol('sol1').feature('t1').feature('se1').feature('ss3').set('linsolver', 'dDef');
model.sol('sol1').feature('t1').feature('se1').feature('ss3').label('Magnetic Particle-Field Interaction 1');
...
model.result('pg3').feature('strmsl1').set('expr', {'mf.Bx' 'mf.By' 'mf.Bz'});
...
model.title('Relativistic Diverging Electron Beam');

model.description('When modeling the propagation of charged particle beams at high currents and relativistic speeds, the space charge and beam current create significant electric and magnetic forces that tend to expand and focus the beam, respectively. Using the Bidirectionally Coupled Particle Tracing study step, it is possible to compute the trajectories and fields in such a way that the two-way coupling between them is considered. In this example, a mesh refinement study is used to confirm that the solution agrees with the analytical expression for the shape of a relativistic beam envelope.');
