...
model.physics('ec').feature('term1').set('V0', '10[kV]');
...
model.sol('sol1').feature('s1').feature('p1').set('plistarr', {'1 10 100 1000 10000 100000'});
...
model.result('pg1').feature('slc2').set('descr', 'Volumetric loss density, electric');
...
model.result.numerical('gev1').setIndex('descr', 'Admittance, 11 component', 0);
...
model.result.table('tbl3').comments('Volume Integration 1');
...
model.title('Frequency Domain Modeling of a Capacitor');

model.description('This example computes the impedance of a capacitor with an applied sinusoidally time-varying voltage difference over a wide frequency range. The relationship between the frequency-domain impedance and the steady-state capacitance and resistance of the device is discussed and solver accuracy is addressed.');
