...
model.selection('sel7').label('Solid');
model.selection('sel7').set([2 3 7 8]);

model.view('view1').set('renderwireframe', true);

model.selection.create('sel8', 'Explicit');
model.selection('sel8').model('comp1');
model.selection('sel8').label('Shell');
model.selection('sel8').geom(2);
model.selection('sel8').set([1 32 37 50 86]);

model.view('view1').set('renderwireframe', false);

model.physics('solid').selection.named('sel7');

model.nodeGroup.create('grp1', 'Physics', 'solid');
model.nodeGroup('grp1').placeAfter('dcont1');
model.nodeGroup('grp1').add('fix1');
model.nodeGroup('grp1').add('bndl1');
model.nodeGroup('grp1').label('Not Used; Applied in Shell Interface');
model.nodeGroup('grp1').active(false);

model.view('view1').set('renderwireframe', true);

model.physics('shell').selection.named('sel8');
model.physics('shell').feature('to1').set('d', '8[mm]');
...
model.physics('shell').feature('el1').set('coordinateSystem', 'LocalEdgeSystem');
model.physics('shell').feature('el1').selection.set([4 188]);
model.physics('shell').feature('el1').set('LoadTypeForce', 'ForceArea');
model.physics('shell').prop('PhysicsSymbols').set('PhysicsSymbols', true);

model.view('view1').set('showDirections', true);

model.physics('shell').feature('el1').set('FeperArea', {'0' 'load(P0*sign(X),Y-PinHoleY,Z)' '0'});

model.multiphysics.create('sshc1', 'SolidShellConnection', 'geom1', -1);
model.multiphysics('sshc1').set('selectionControl', true);
model.multiphysics('sshc1').selection('edgBndSolidSelection').set([9 11 13 14 30 34 58 62 80 81 83 84]);
model.multiphysics('sshc1').set('connectionType', 'Flexible');
...
model.sol('sol1').feature('v1').feature('comp1_sshc1_qsh').set('scalemethod', 'manual');
...
model.sol('sol1').feature('v1').feature('comp1_sshc1_qsh').set('scaleval', '1e-5');
model.sol('sol1').feature('v1').set('control', 'stat');
model.sol('sol1').create('s1', 'Stationary');
model.sol('sol1').feature('s1').feature('aDef').set('cachepattern', true);
...
model.result('pg1').run;
model.result('pg2').run;
model.result('pg2').label('Stress, Solid + Shell');
model.result('pg2').feature.copy('vol1', 'pg1/vol1');
model.result('pg2').run;
...
model.result('pg2').feature('surf1').set('rangecolormax', 70);
model.result('pg2').run;
model.result.create('pg3', 'PlotGroup3D');
model.result('pg3').set('data', 'dset1');
model.result('pg3').set('defaultPlotID', 'shellGeometry');
model.result('pg3').label('Shell Geometry (shell)');
model.result('pg3').set('titletype', 'label');
model.result('pg3').set('showlegends', false);
model.result('pg3').set('edgecolor', 'cyan');
model.result('pg3').create('surf1', 'Surface');
model.result('pg3').feature('surf1').set('expr', {'if(abs(shell.z)==1,shell.z, NaN)'});
...
model.result('pg3').feature('surf1').set('data', 'dset1shellshl');
model.result('pg3').feature('surf1').label('Top and Bottom');
model.result('pg3').label('Shell Geometry (shell)');
model.result('pg3').run;
model.result.create('pg4', 'PlotGroup3D');
model.result('pg4').set('data', 'dset1');
model.result('pg4').set('defaultPlotID', 'thicknessOrientation');
model.result('pg4').label('Thickness and Orientation (shell)');
model.result('pg4').set('titletype', 'label');
model.result('pg4').set('showlegendsunit', true);
model.result('pg4').create('surf1', 'Surface');
model.result('pg4').feature('surf1').set('expr', {'shell.d'});
...
model.result('pg4').feature('surf1').label('Thickness');
model.result('pg4').create('syss', 'CoordSysSurface');
model.result('pg4').feature('syss').set('sys', 'shellsys');
model.result('pg4').feature('syss').label('Shell Local System');
model.result('pg4').label('Thickness and Orientation (shell)');
model.result('pg4').run;
model.result('pg3').run;
model.result('pg3').run;
...
model.result('pg5').run;
model.result('pg5').label('Principal Strain');
model.result('pg5').run;
model.result('pg5').feature('surf1').set('expr', 'shell.ep1');
model.result('pg5').feature('surf1').set('rangecolormax', 2.0E-4);
model.result('pg5').feature('surf1').set('colortabletype', 'discrete');
model.result('pg5').run;
model.result('pg5').feature('vol1').set('expr', 'solid.ep1');
model.result('pg5').run;

model.title(['Bracket ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' Shell Analysis']);

model.description('This example computes the stresses within a bracket subjected to external loads using the Shell interface.');

model.mesh.clearMeshes;

model.sol('sol1').clearSolutionData;

model.label('bracket_shell.mph');
