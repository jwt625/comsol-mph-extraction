function out = model
%
% waveguide_filter_optimization_polynomial.m
%
...
model.geom('geom1').insertFile('waveguide_filter_optimization_polynomial_geom_sequence.mph', 'geom1');
...
model.physics('emw').feature('port1').selection.named('geom1_boxsel4');
model.physics('emw').feature('port1').set('PortType', 'Rectangular');
model.physics('emw').create('port2', 'Port', 1);
model.physics('emw').feature('port2').selection.named('geom1_boxsel5');
...
model.common('poly1').selection.named('geom1_boxsel3');
model.common('poly1').set('maximumDisplacement', 'w_wg/6');
model.common('poly1').set('order', '1');
...
model.mesh('mesh1').stat.selection.set([8 10 12]);
...
model.sol('sol1').feature('v1').feature('comp1_material_disp').set('scaleval', '0.0032002249920903996');
...
model.result('pg4').feature('arwl1').set('expr', {'fsd1.dXg' 'fsd1.dYg'});
...
model.title(['Optimization of a Waveguide Iris Bandpass Filter ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' Polynomial Version']);

model.description('A waveguide filter is designed using shape optimization by moving the vertices in the geometry. The irises of the initial geometry are optimized to ensure good bandpass response and out-of-band rejection, while maintaining the double mirror symmetry.');
