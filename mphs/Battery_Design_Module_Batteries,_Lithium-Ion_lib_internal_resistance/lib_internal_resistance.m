...
model.param.set('soc_init', '50[%]');
model.param.create('par2');
model.param('par2').label('Parameters - Pulse');

% To import content from file, use:
% model.param('par2').loadFile('FILENAME');
model.param('par2').set('t_pulse', '10[s]', 'Pulse time');
model.param('par2').set('t_rest', '20[s]', 'Rest time between pulses');
model.param('par2').set('I_pulse', '10[A]', 'Cell current during pulse');
...
model.physics('ev').feature('ds1').setIndex('dim', 'CH_DCH', 0, 0);
model.physics('ev').feature('ds1').setIndex('dimInit', 0, 0, 0);
model.physics('ev').feature('ds1').setIndex('dimInit', -1, 0, 0);
model.physics('ev').feature('ds1').setIndex('dimDescr', 'State variable (=-1 for discharge, =0 for rest, =+1 for charge)', 0, 0);
model.physics('ev').create('es1', 'EventSequence', -1);
model.physics('ev').feature('es1').feature('sm1').set('stateName', 'DCH');
model.physics('ev').feature('es1').feature('sm1').set('endConditionOptions', 'duration');
model.physics('ev').feature('es1').feature('sm1').set('duration', 't_pulse');
model.physics('ev').feature('es1').feature('sm1').setIndex('reInitName', 'CH_DCH', 0, 0);
model.physics('ev').feature('es1').feature('sm1').setIndex('reInitValue', 0, 0, 0);
model.physics('ev').feature('es1').create('sm2', 'SequenceMember', -1);
model.physics('ev').feature('es1').feature('sm2').set('stateName', 'REST');
model.physics('ev').feature('es1').feature('sm2').set('endConditionOptions', 'duration');
model.physics('ev').feature('es1').feature('sm2').set('duration', 't_rest');
model.physics('ev').feature('es1').feature('sm2').setIndex('reInitName', 'CH_DCH', 0, 0);
model.physics('ev').feature('es1').feature('sm2').setIndex('reInitValue', 0, 0, 0);
model.physics('ev').feature('es1').feature('sm2').setIndex('reInitValue', 1, 0, 0);

model.variable('var1').set('I_app', 'I_pulse/A_cell*CH_DCH');
model.variable('var1').descr('I_app', 'Applied current density');
model.variable('var1').set('R_internal', 'E_pol_tot/(I_app*A_cell)');
model.variable('var1').descr('R_internal', 'R_internal');
model.variable('var1').set('C_rate', 'I_app/I_1C');
model.variable('var1').descr('C_rate', 'C rate');

model.physics('liion').feature('ecd1').set('nis', 'I_app');

model.study('std1').feature('time').set('tunit', 's');
model.study('std1').feature('time').set('tlist', 'range(0,t_pulse/10,2*t_pulse+t_rest)');
...
model.study('std1').feature('param').setIndex('plistarr', '', 0);
model.study('std1').feature('param').setIndex('punit', 'm^2', 0);
model.study('std1').feature('param').setIndex('pname', 'L_pos', 0);
model.study('std1').feature('param').setIndex('plistarr', '25 60', 0);
...
model.sol('sol1').feature('v2').set('control', 'time');
model.sol('sol1').create('t1', 'Time');
model.sol('sol1').feature('t1').set('tlist', 'range(0,t_pulse/10,2*t_pulse+t_rest)');
model.sol('sol1').feature('t1').set('plot', 'off');
model.sol('sol1').feature('t1').set('plotgroup', 'Default');
model.sol('sol1').feature('t1').set('plotfreq', 'tout');
model.sol('sol1').feature('t1').set('probesel', 'all');
...
model.batch('p1').set('pname', {'L_pos'});
model.batch('p1').set('plistarr', {'25 60'});
...
model.result('pg6').label('Electrolyte Salt Concentration (liion)');
model.result('pg2').run;
model.result('pg2').label('Cell Voltage and Current');
model.result('pg2').create('glob1', 'Global');
model.result('pg2').feature('glob1').set('markerpos', 'datapoints');
model.result('pg2').feature('glob1').set('linewidth', 'preference');
model.result('pg2').feature('glob1').set('expr', {'C_rate'});
model.result('pg2').feature('glob1').set('descr', {'C rate'});
...
model.result('pg2').feature('ptgr1').set('legendsuffix', ', E_cell');
model.result('pg2').run;
...
model.result('pg7').label('Internal Resistance');
model.result('pg7').set('data', 'dset3');
model.result('pg7').create('glob1', 'Global');
model.result('pg7').feature('glob1').set('markerpos', 'datapoints');
model.result('pg7').feature('glob1').set('linewidth', 'preference');
model.result('pg7').feature('glob1').set('expr', {'R_internal'});
model.result('pg7').feature('glob1').set('descr', {'R_internal'});
model.result('pg7').feature('glob1').set('unit', {['ohm' ]});
model.result('pg7').feature('glob1').set('autodescr', false);
model.result('pg7').run;
...
model.variable('var2').set('E_act_neg', 'intop_neg(liion.eta_per1*liion.iv_per1)/I_app', 'Electrode activation voltage loss, negative');
model.variable('var2').set('E_l_neg', 'intop_neg(-philx*liion.Ilx)/I_app', 'Electrolyte ohmic voltage loss, negative');
model.variable('var2').set('E_s_neg', 'intop_neg(-phisx*liion.Isx)/I_app', 'Electrode ohmic voltage loss, negative');
model.variable('var2').set('E_ocp_loc_neg', 'mat2.elpot.Eeq_int1(liion.socloc_average)', 'Particle average open circuit potential, negative');
model.variable('var2').set('E_conc_particle_neg', 'intop_neg((liion.Eeq_per1-E_ocp_loc_neg)*liion.iv_per1)/I_app', 'Particle concentration voltage loss, negative');
model.variable('var2').set('E_conc_electrode_neg', 'intop_neg((E_ocp_loc_neg-E_ocp_neg)*liion.iv_per1)/I_app', 'Electrode concentration voltage loss, negative');
model.variable('var2').set('E_conc_neg', 'E_conc_electrode_neg+E_conc_particle_neg', 'Concentration voltage loss, negative');
model.variable('var2').set('E_l_sep', 'intop_sep(-philx*liion.Ilx)/I_app', 'Electrolyte ohmic voltage loss, separator');
model.variable('var2').set('E_act_pos', 'intop_pos(liion.eta_per1*liion.iv_per1)/I_app', 'Electrode activation voltage loss, positive');
model.variable('var2').set('E_l_pos', 'intop_pos(-philx*liion.Ilx)/I_app', 'Electrolyte ohmic voltage loss, positive');
model.variable('var2').set('E_s_pos', 'intop_pos(-phisx*liion.Isx)/I_app', 'Electrode ohmic voltage loss, positive');
model.variable('var2').set('E_ocp_loc_pos', 'mat3.elpot.Eeq_int1(liion.socloc_average)', 'Particle average open circuit potential, positive');
model.variable('var2').set('E_conc_particle_pos', 'intop_pos((liion.Eeq_per1-E_ocp_loc_pos)*liion.iv_per1)/I_app', 'Particle concentration voltage loss, positive');
model.variable('var2').set('E_conc_electrode_pos', 'intop_pos((E_ocp_loc_pos-E_ocp_pos)*liion.iv_per1)/I_app', 'Electrode concentration voltage loss, positive');
model.variable('var2').set('E_conc_pos', 'E_conc_electrode_pos+E_conc_particle_pos', 'Concentration voltage loss, positive');
model.variable('var2').set('E_neg', 'E_act_neg+E_l_neg+E_s_neg+E_conc_particle_neg+E_conc_electrode_neg', 'Voltage loss, negative');
model.variable('var2').set('E_sep', 'E_l_sep', 'Voltage loss, separator');
model.variable('var2').set('E_pos', 'E_act_pos+E_l_pos+E_s_pos+E_conc_particle_pos+E_conc_electrode_pos', 'Voltage loss, positive');

model.cpl.create('intop1', 'Integration', 'geom1');
model.cpl('intop1').set('axisym', true);
model.cpl('intop1').set('opname', 'intop_neg');
model.cpl('intop1').selection.named('sel2');
model.cpl.create('intop2', 'Integration', 'geom1');
model.cpl('intop2').set('axisym', true);
model.cpl('intop2').set('opname', 'intop_sep');
model.cpl('intop2').selection.named('sel1');
model.cpl.create('intop3', 'Integration', 'geom1');
model.cpl('intop3').set('axisym', true);
model.cpl('intop3').set('opname', 'intop_pos');
...
model.result('pg8').label('Voltage Losses, All Domains');
model.result('pg8').create('glob1', 'Global');
model.result('pg8').feature('glob1').set('markerpos', 'datapoints');
model.result('pg8').feature('glob1').set('linewidth', 'preference');
model.result('pg8').feature('glob1').set('data', 'dset3');
model.result('pg8').feature('glob1').setIndex('looplevelinput', 'manual', 1);
model.result('pg8').feature('glob1').setIndex('looplevel', [1], 1);
model.result('pg8').feature('glob1').set('expr', {'E_neg'});
model.result('pg8').feature('glob1').set('descr', {'Voltage loss, negative'});
model.result('pg8').feature('glob1').set('expr', {'E_neg' 'E_sep'});
model.result('pg8').feature('glob1').set('descr', {'Voltage loss, negative' 'Voltage loss, separator'});
model.result('pg8').feature('glob1').set('expr', {'E_neg' 'E_sep' 'E_pos'});
model.result('pg8').feature('glob1').set('descr', {'Voltage loss, negative' 'Voltage loss, separator' 'Voltage loss, positive'});
model.result('pg8').feature('glob1').set('autosolution', false);
model.result('pg8').feature('glob1').set('autodescr', false);
model.result('pg8').feature('glob1').set('autoexpr', true);
model.result('pg8').feature.duplicate('glob2', 'glob1');
model.result('pg8').run;
model.result('pg8').feature('glob2').setIndex('looplevel', [2], 1);
...
model.result('pg8').set('title', 'Solid Lines: Power optimized cell, Dashed Lines: Energy optimized cell');
...
model.result('pg9').label('Voltage Losses, Negative Electrode');
...
model.result('pg9').feature('glob1').setIndex('looplevelinput', 'manual', 1);
model.result('pg9').feature('glob1').setIndex('looplevel', [1], 1);
model.result('pg9').feature('glob1').set('expr', {'E_s_neg'});
model.result('pg9').feature('glob1').set('descr', {'Electrode ohmic voltage loss, negative'});
model.result('pg9').feature('glob1').set('unit', {'V'});
model.result('pg9').feature('glob1').set('expr', {'E_s_neg' 'E_l_neg'});
model.result('pg9').feature('glob1').set('descr', {'Electrode ohmic voltage loss, negative' 'Electrolyte ohmic voltage loss, negative'});
model.result('pg9').feature('glob1').set('expr', {'E_s_neg' 'E_l_neg' 'E_act_neg'});
model.result('pg9').feature('glob1').set('descr', {'Electrode ohmic voltage loss, negative' 'Electrolyte ohmic voltage loss, negative' 'Electrode activation voltage loss, negative'});
model.result('pg9').feature('glob1').set('expr', {'E_s_neg' 'E_l_neg' 'E_act_neg' 'E_conc_neg'});
model.result('pg9').feature('glob1').set('descr', {'Electrode ohmic voltage loss, negative' 'Electrolyte ohmic voltage loss, negative' 'Electrode activation voltage loss, negative' 'Concentration voltage loss, negative'});
model.result('pg9').run;
model.result('pg9').feature('glob1').set('autosolution', false);
model.result('pg9').feature('glob1').set('autodescr', false);
model.result('pg9').feature('glob1').set('autoexpr', true);
model.result('pg9').feature.duplicate('glob2', 'glob1');
model.result('pg9').run;
model.result('pg9').feature('glob2').setIndex('looplevel', [2], 1);
model.result('pg9').feature('glob2').set('linestyle', 'dashed');
model.result('pg9').feature('glob2').set('linecolor', 'cyclereset');
model.result('pg9').feature('glob2').set('legend', false);
model.result('pg9').run;
model.result('pg9').set('ylabelactive', true);
model.result('pg9').set('ylabel', 'Voltage (V)');
model.result('pg9').set('titletype', 'manual');
model.result('pg9').set('title', 'Solid Lines: Power optimized cell, Dashed Lines: Energy optimized cell');
model.result('pg9').set('legendpos', 'upperleft');
model.result('pg9').run;
model.result.duplicate('pg10', 'pg9');
model.result('pg10').run;
model.result('pg10').label('Voltage Losses, Positive Electrode');
model.result('pg10').run;
model.result('pg10').feature('glob1').setIndex('expr', 'E_s_pos', 0);
model.result('pg10').feature('glob1').setIndex('expr', 'E_l_pos', 1);
model.result('pg10').feature('glob1').setIndex('expr', 'E_act_pos', 2);
model.result('pg10').feature('glob1').setIndex('expr', 'E_conc_pos', 3);
model.result('pg10').run;
model.result('pg10').feature('glob2').setIndex('expr', 'E_s_pos', 0);
model.result('pg10').feature('glob2').setIndex('expr', 'E_l_pos', 1);
model.result('pg10').feature('glob2').setIndex('expr', 'E_act_pos', 2);
model.result('pg10').feature('glob2').setIndex('expr', 'E_conc_pos', 3);
model.result('pg10').feature('glob2').setIndex('descr', 'Particle concentration voltage loss, positive', 3);
model.result('pg10').run;
model.result('pg10').run;

model.title('Lithium-Ion Battery Internal Resistance');

model.description('This example demonstrates the Lithium-Ion Battery interface for simulation of the internal resistance and voltage loss when high current pulses are applied in a hybrid pulse power characterization (HPPC) test. Contribution to the losses from different parts of the battery are also analyzed when varying different design parameters.');
...
model.label('lib_internal_resistance.mph');
