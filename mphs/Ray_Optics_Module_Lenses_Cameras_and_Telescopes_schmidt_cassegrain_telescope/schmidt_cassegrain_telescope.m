function out = model
%
% schmidt_cassegrain_telescope.m
%
...
model.param('par2').set('lam1', '486[nm]', 'Wavelength 1');
model.param('par2').set('lam2', '546[nm]', 'Wavelength 2');
model.param('par2').set('lam3', '656[nm]', 'Wavelength 3');
...
model.param('par2').set('theta_x2', '0.125[deg]', 'Field angle 2, x-component');
model.param('par2').set('theta_y2', '0.000[deg]', 'Field angle 2, y-component');
model.param('par2').set('theta_x3', '0.250[deg]', 'Field angle 3, x-component');
model.param('par2').set('theta_y3', '0.000[deg]', 'Field angle 3, y-component');
model.param('par2').set('N_ring', '12', 'Number of hexapolar rings');
model.param('par2').set('P_nom', '203.2[mm]', 'Nominal entrance pupil diameter');
...
model.param('par2').set('dz', '20[mm]', 'Ray launch z-coordinate');
...
model.geom('geom1').label('Schmidt-Cassegrain Telescope');
model.geom('geom1').insertFile('schmidt_cassegrain_telescope_geom_sequence.mph', 'geom1');
...
model.physics('gop').feature('mir1').selection.named('geom1_csel3_bnd');
...
model.physics('gop').feature('wall2').selection.named('geom1_pi4_sel1');
...
model.study('std1').feature('rtrac').set('llist', '0 1250');
...
model.result('pg1').feature('surf1').feature('sel1').selection.named('geom1_csel3_bnd');
model.result('pg1').run;
...
model.result('pg1').feature('surf2').set('customcolor', [0.4117647111415863 0.4117647111415863 0.4117647111415863]);
...
model.result('pg1').run;
model.result('pg1').feature('surf3').set('customcolor', [0.21176470816135406 0.5490196347236633 0.7960784435272217]);
...
model.title(['Schmidt' native2unicode(hex2dec({'20' '13'}), 'unicode') 'Cassegrain Telescope']);

model.description(['The Schmidt' native2unicode(hex2dec({'20' '13'}), 'unicode') 'Cassegrain Telescope demonstrated in this tutorial is a classic design using an aspheric corrector lens and two spherical mirrors. The aspheric corrector is created using the ''Aspheric Even Lens 3D'' part from the Ray Optics Module Part Library.']);

model.mesh.clearMeshes;

model.sol('sol1').clearSolutionData;

model.label('schmidt_cassegrain_telescope.mph');
