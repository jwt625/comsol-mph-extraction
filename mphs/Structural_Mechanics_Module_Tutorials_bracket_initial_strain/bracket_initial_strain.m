...
model.param.set('L0', '215[mm]');
model.param.descr('L0', 'Initial pin length');
model.param.set('L', '214[mm]');
model.param.descr('L', 'Current pin length');
model.param.set('InitStrain', '(L-L0)/L0');
model.param.descr('InitStrain', 'Pin strain');

model.geom('geom1').create('imp2', 'Import');
model.geom('geom1').feature('imp2').set('type', 'native');
model.geom('geom1').feature('imp2').set('filename', 'bracket_pin.mphbin');
...
model.physics('solid').feature('lemm1').feature('iss1').set('eil', {'InitStrain' '0' '0' '0' '0' '0' '0' '0' '0'});

model.mesh('mesh1').feature('ftet1').feature('size1').selection.geom('geom1', 3);
model.mesh('mesh1').feature('ftet1').feature('size1').selection.set([5 6 10 11]);
model.mesh('mesh1').run;
...
model.result('pg2').label('Third Principal Strain');
model.result('pg2').create('surf1', 'Surface');
model.result('pg2').feature('surf1').set('expr', 'solid.ep3');
model.result('pg2').feature('surf1').set('descr', 'Third principal strain');
model.result('pg2').run;
...
model.view('view1').clip('plane1').set('position', [0 -0.17499999701976773 0]);
model.view('view1').clip('plane1').set('translationamount', 0.010700000137090684);
model.view('view1').set('clipshowgizmos', false);

model.title(['Bracket ' native2unicode(hex2dec({'20' '14'}), 'unicode') ' Initial-Strain Analysis']);

model.description('This example computes the stress distribution in a structural bracket connected to a prestrained pin.');

model.mesh.clearMeshes;

model.sol('sol1').clearSolutionData;

model.label('bracket_initial_strain.mph');
