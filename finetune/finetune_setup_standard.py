#!/usr/bin/env python3
"""
COMSOL Fine-Tuning Setup using Standard HuggingFace Training
Alternative approach to avoid SFTTrainer issues
"""
...
def prepare_dataset(train_data, tokenizer, max_length=2048):
    """Prepare and tokenize dataset for training"""
    
    # Format all examples
    texts = [format_training_text(example) for example in train_data]
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune model on COMSOL data (Standard HF)")
    ...
    try:
        ...
        training_args = TrainingArguments(
            ...
            warmup_steps=min(100, len(tokenized_dataset) // 10),
            ...
            seed=3407,
            save_steps=min(500, len(tokenized_dataset) // 4),
            save_total_limit=3,
